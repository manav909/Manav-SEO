...
Build 10b — File upload (PDF / DOCX / XLSX) [PLANNED, not yet built]: Adds Supabase Storage integration + server-side parsers (pdfjs-dist/pdf-parse, mammoth, xlsx) so the operator can upload reference files instead of pasting text. Was deliberately deferred from 10a to keep that build scoped — parsing edge cases (encrypted PDFs, OCR for scans, complex spreadsheets) are real engineering. Build 10a delivers ~80% of value via the paste-text approach; 10b hardens the workflow once 10a is proven in real use.
