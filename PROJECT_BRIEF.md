...
**Chrome extension — Fiverr Lead Cockpit (v1.6.0) [BUILT 2026-06-09].** Standalone MV3 extension at repo root in `fiverr-lead-evaluator/` (OUTSIDE the Vite build; api stays 12; frontend TS held at 47). Reads the open Fiverr conversation live and POSTs to `/api/task-engine` (no auth header) via the background worker. **v1.6 = whole-inbox board + paced review** (safe path Manav chose: reads the on-page list only, NO private endpoints, NO silent bulk scraping). Panel now has a header toggle between two views: **Chat** (the per-lead cockpit) and **Inbox** (the board). **Inbox board:** `loadInbox()` → `scanInbox()` auto-scrolls the conversation-list scroll container (found by walking up from an `a[href*="/inbox/"]` to the first overflow-auto/scroll ancestor) until row count stabilizes (3 stable scans) or 60 steps, collecting rows via `inboxRows()` (anchored on the `/inbox/<handle>` href — class-independent; name=first text line, snippet=rest, unread=heuristic class/badge probe); then NEW read-only `bd_deal_lookup({handles[],platform})` (bulk handle→{stage,temperature,status,evaluated,has_intel,last_message_at}, no create, no LLM) enriches each row; `sortInbox` orders unread-first then hot→warm→cold then recent. Rows show name + snippet + stage pill + hot/warm/cold (or "needs reply" from the unread marker); unworked leads show raw inbox until opened. Tap a row → `openLead()` navigates (clicks the row's anchor / sets location), resets chat state, and re-resolves identity (`findDeal`→`evaluate`) once `clientHandle()` matches. **Paced review:** `startReview()` queues all handles; `reviewStep()` opens each conversation, waits for it to load (handle match or ~14 tries), `findDeal`→`evaluate` (persists strategy to that deal), then advances after `reviewDelay` (9s, human-speed); Pause/Resume/Stop. **Guards:** `sameClient()` tightened to return false when no conversation handle is open (so browsing the inbox list never autosaves junk); `renderBody()` no-ops when `view!=="chat"` so background autosave/eval callbacks never clobber the board. BACKEND change this build: NEW `bd_deal_lookup` action (read-only) in handleBd/bd-engine.ts. Plus all prior cockpit features (autosave/sync, say-this-next + tone + expectations, expert ⚡ triggers, live SEO ops, saved intel, docs, reply). KNOWN LIMITS: inbox list/scroll detection + message-box + site detection are heuristic (Fiverr markup non-public/unstable — may need the live inbox-row element to hard-target); virtualized list means the board reads what auto-scroll loads; paced review = one LLM call per lead; pdf/image parsing not built; expert not yet fed `algorithm_knowledge`/`brain_learnings`. DEPLOY: touches BACKEND (`api/lib/bd-engine.ts`) → real Vercel deploy + force-rebuild, PLUS reload the unpacked extension locally.
