...
**Chrome extension — Fiverr Lead Evaluator (v1.0.0) [BUILT 2026-06-09].** Standalone MV3 extension living at repo root in `fiverr-lead-evaluator/` (OUTSIDE the Vite build — `tsconfig.app.json` only includes `src`, the JS is not in `api/`, so it adds 0 serverless functions and does not affect `vite build`). It reads the open Fiverr conversation from the live page (logged-in session, no clipboard) and POSTs it to `/api/task-engine` (`bd_strategize`, no auth header — same as the web app) via the background service worker (host_permissions bypass page CORS), then renders the evaluation in a floating shadow-DOM panel: deal stage/temperature, summary, next move, do-now, wants/pain-points/objections/buying-signals, risks, captured facts, call script. Files: manifest.json, background.js (generic `callEngine` proxy), content.js (heuristic scraper + "Use selection" fallback + panel), popup.html/js (configurable API base, default seoseason.com), README.md (install). Loaded as unpacked. KNOWN LIMIT: Fiverr DOM markup is non-public/unstable, so the auto-scraper is best-effort (most text-dense conversation container) with a manual selection fallback; hard-targeting needs the live conversation container's CSS class. Currently runs lead evaluation only — order ingestion / documents / engagement can be added as further `action` calls. NOTE: not a Vercel deploy (browser extension loaded locally); committed to the repo for version control.
