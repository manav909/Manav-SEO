...
**Chrome extension — Fiverr Lead Cockpit (v1.5.0) [BUILT 2026-06-09].** Standalone MV3 extension at repo root in `fiverr-lead-evaluator/` (OUTSIDE the Vite build — `tsconfig.app.json` only includes `src`, JS not in `api/`, 0 serverless functions, `vite build` unaffected; api stays 12; frontend TS held at 47). Reads the open Fiverr conversation live (logged-in session, no clipboard) and POSTs to `/api/task-engine` (no auth header) via the background worker (host_permissions bypass CORS), shadow-DOM panel. **v1.5 = always-in-sync autosave + proactive senior-DMS cockpit** (built for a non-technical operator who does not know what to ask). **Sync loop:** each Fiverr client = one `bd_deals` row keyed by `/inbox/<handle>` (via `bd_deal_find`, build v1.4). On open the panel resolves the deal and `watchChat()` attaches a MutationObserver (+25s backup poll for virtualized lists) to the densest conversation container; on change (debounced 3s) it `autosave()`s the chat to the deal via NEW partial `bd_deal_update` conversation field (clobber-free — only sets conversation+last_message_at, does NOT reset status/strategy/tags), shows "✓ synced Xs" in the header, and `maybeAutoEval()` re-strategizes automatically when the chat grew >200 chars AND >90s since last eval (rate-limited so it stays in sync without burning LLM calls). **Proactive surface (don't echo Fiverr-visible info):** read-out now LEADS with `draft_reply` ("Say this next" — editable + Insert into Fiverr, surfaced from the strategy with NO extra call; draft_reply was already produced by strategizeDeal but never shown), then Why/next move, **Client tone**, **Their expectations**, Watch out, Objections to handle, Do now (action_items, with "platform can run this" tag when platform_can_help). Summary/wants/facts/call-script demoted into a collapsed "More detail". **Expert triggers actions:** `bd_ask`'s `suggested_tools` are mapped (mapTool: competitor/aeo/audit) and rendered as ⚡ one-tap run-it buttons that fire the matching live op, save to the deal, and feed the next answer. **Live SEO ops** (build v1.4): Crawl & audit / AEO / Competitor gap, server-side, saved to deal, shown in Saved intel. Documents & calls collapsed. ✍ Reply (3 angles) + Evaluate/Sel. **BACKEND changes this build:** (a) `bd_deal_update` accepts a partial `conversation` (autosave); (b) **strategizeDeal enriched with `client_intel.tone` + top-level `expectations`** (interface + SYSTEM bullet + JSON schema + defaults + parser) — this also flows to the web `/deals` read-out, not just the extension. **Insertion:** React-aware (native value setter + input events for textarea; execCommand insertText for contenteditable), heuristic box; NEVER auto-sends; Copy fallback. KNOWN LIMITS: heuristic message-box + site detection; raw conversation now persisted via autosave; pdf/image parsing not built; expert fed saved crawl/AEO/competitor intel but NOT yet `algorithm_knowledge`/`brain_learnings`. DEPLOY: touches BACKEND (`api/lib/bd-engine.ts`, `api/lib/bd-strategist.ts`) → real Vercel deploy + force-rebuild, PLUS reload the unpacked extension locally.
