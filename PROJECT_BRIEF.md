...
- NEXT for reports (not yet built): per-stage individual client docs (current = one consolidated report + per-stage JSON); richer formatting / docx or PDF export of the consolidated report; optional bounded LLM polish of the prose (kept deterministic for now to stay fully grounded). Registry now ~20 capabilities. Honest sub-gaps remaining by design (need external data, never faked): competitor backlink-gap (needs a backlink data source), and a full Google Ads OAuth connector (CSV ingestion is the current real path). FIELD VALIDATION owed across all session engines — run real briefs/projects and confirm outputs hold up before client use.
