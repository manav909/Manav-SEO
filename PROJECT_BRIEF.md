...
**Chrome extension — Fiverr Lead Evaluator (v1.4.0) [BUILT 2026-06-09].** Standalone MV3 extension at repo root in `fiverr-lead-evaluator/` (OUTSIDE the Vite build — `tsconfig.app.json` only includes `src`, JS not in `api/`, 0 serverless functions, `vite build` unaffected; api stays 12). Reads the open Fiverr conversation live (logged-in session, no clipboard) and POSTs to `/api/task-engine` (no auth header) via the background worker (host_permissions bypass CORS), rendering a shadow-DOM panel. **v1.4 = persistent client memory + live SEO ops** (the analyser/expert alone could not save or reuse anything). **The loop:** each Fiverr client maps to ONE `bd_deals` row, keyed by the conversation handle in the URL (`/inbox/<handle>` — stable, not Fiverr-markup-dependent), via NEW backend action `bd_deal_find` (find-or-create by client_handle+platform, in handleBd/bd-engine.ts, no new function slot). On panel open the extension resolves that deal (`findDeal` → `bd_deal_find`) and carries its `id` into every call. Because `bd_strategize` (with id) PERSISTS the full strategy (deal_facts/stage/status/industry/country/client_type + stage_history) and `dealContext` (used by `bd_ask`/`bd_reply_variants`) + `bd_strategize` PULL all deal attachments+facts+summary back into the prompt, and the SEO ops persist their summaries as attachments — the entire "take every detail to the software, save forever, fetch on the go, feed the expert" loop works by passing one id (all backend support pre-existed; this build wires the id + identity primitive). **Panel (top→bottom):** (1) Expert toolkit (`bd_ask`) — command bar + 7 quick actions + ▶ do-this chips from `action_items`; Expert take + Ready-to-send → Insert into Fiverr. (2) **Saved intel** — the deal's stored attachments (crawl/AEO/competitor/facts), fetched on open, shown as expandable rows; "reused next time + fed to every expert answer." (3) **Live SEO ops** — client-site field (auto-detected from chat via `detectSite`, skips fiverr/social/util domains) + **Crawl & audit** (`bd_run_audit`), **AEO/schema** (`bd_aeo_check`), **Competitor gap** (`bd_competitor_snapshot` with competitor-domain + keyword inputs); heavy lifting server-side (crawlSite/SerpAPI), summaries saved onto the deal, `refreshDeal` (`bd_deal_get`) re-pulls Saved intel after each op. (4) Documents & calls — NOW COLLAPSED by default (▸ toggle, count badge); drop attachments/transcripts, parsed in-browser (xlsx/csv via SheetJS, docx via mammoth, txt/md/json; pdf/images not yet). (5) ✍ Reply (`bd_reply_variants`, 3 angles) + Evaluate/Sel read-out. **Insertion:** React-aware (native value setter + input events for textarea; `execCommand('insertText')` for contenteditable), box found heuristically (lowest visible text input); NEVER auto-sends (human-in-the-loop for account/ToS safety); Copy fallback. Files: manifest.json, background.js, content.js, popup.html/js, vendor-xlsx.js (~864K), vendor-mammoth.js (~624K), README.md. KNOWN LIMITS: heuristic detection + message-box targeting; saved raw conversation NOT persisted (live chat always re-read); pdf/image parsing not built. DEPLOY NOTE: this build touches BACKEND (`api/lib/bd-engine.ts` — `bd_deal_find`) → real Vercel deploy + force-rebuild required, PLUS reload the unpacked extension locally.
