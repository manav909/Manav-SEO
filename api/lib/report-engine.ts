...
export async function generateReport(projectId:string,reportType:"weekly"|"monthly"|"quarterly"){
  ...
  const ai=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":process.env.ANTHROPIC_API_KEY||"","anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2500,messages:[{role:"user",content:`Write a professional ${reportType} SEO report as HTML body (no html/head tags) for ${project.name} (${project.url||""}). Goals: ${project.goals||"improve organic visibility"}. Period: last ${daysBack} days. Tasks done: ${done}. Learnings: ${learns.length}. Wins: ${wins.slice(0,3).join(", ")||"building"}. Style: dark #070710 bg, #1e1e3a card borders, #6366f1 accent, white text, inline CSS. Sections: Executive Summary, Work Completed, Key Wins, What We Learned, Next Steps.`}]})});
