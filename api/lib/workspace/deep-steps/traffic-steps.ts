...
export async function gatherQueryLandscape(opts: { projectId: string; targetUrls: string[] }): Promise<StepResult> {
  ...
  const evidence = { step_key: "query_landscape", generated_at: new Date().toISOString(), wrong_page: wrongPage, high_impr_low_ctr: highImprLowCtr, serp_landscape: serpLandscape, provenance, worth_deeper };
  ...
  for (const s of serpLandscape) { L.push(`\n**"${s.query}"** (pos ${s.position.toFixed(1)}, ${s.impressions} impr) — features: ${s.features.join(", ") || "none"}`); if (s.paa.length) { L.push(`PAA:`); for (const q of s.paa) L.push(`- ${q}`); } }
