...
export const GOAL_DEFS: Record<string, GoalDef> = {
  keyword_ranking: {
    ...
    needs: ["gsc_visibility", "competitor_intel", "query_landscape", "onpage_audit"],
    pillars: ["visibility", "query_opportunity", "on_page_health"],
    panel_framing: "Focus on closing the gap to page 1 for target keywords: what competitors ranking above have, on-page deficits, and the fastest ranking levers.",
  },
  page_growth: {
    id: "page_growth", label: "Page Growth",
    description: "Grow organic performance of specific target pages.",
    needs: ["gsc_visibility", "onpage_audit", "competitor_intel", "internal_link_graph"],
    pillars: ["visibility", "on_page_health", "internal_links"],
    panel_framing: "Focus on what holds each target page back — indexation, on-page quality, internal link support, competitive gap.",
  },
  traffic_growth: {
  ...
  conversion: {
    ...
    needs: ["engagement_value", "gsc_visibility", "onpage_audit"],
    pillars: ["engagement", "on_page_health"],
    panel_framing: "Focus on where organic visitors arrive but fail to convert, and the highest-value pages to fix first.",
  },
  authority_eeat: {
    id: "authority_eeat", label: "Authority & E-E-A-T",
    description: "Build domain and page authority / E-E-A-T signals.",
    needs: ["authority_signals", "competitor_intel", "onpage_audit"],
    pillars: ["on_page_health"],
    panel_framing: "Assess off-site authority and on-page E-E-A-T against competitors; identify credible, verifiable authority-building moves.",
  },
  topical_authority: {
    ...
    needs: ["query_landscape", "competitor_intel", "onpage_audit", "internal_link_graph"],
    pillars: ["query_opportunity", "on_page_health", "internal_links"],
    panel_framing: "Map the topic's query space, find coverage gaps vs. competitors, and design the cluster + internal linking to own it.",
  },
};
