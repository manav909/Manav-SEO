      ...
      <div style={{position:"relative",zIndex:1,maxWidth:1000,margin:"0 auto",
        ...
        <div style={{textAlign:"center" as const,marginBottom:52}}>
          <div style={{fontFamily:"var(--font-mono)",fontSize:10,letterSpacing:"3px",
            textTransform:"uppercase" as const,color:"hsl(var(--muted-foreground))",marginBottom:12}}>
          ...
          <div style={{fontSize:15,color:"hsl(var(--muted-foreground))",maxWidth:500,margin:"0 auto",lineHeight:1.7}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center" as const,
            gap:8,marginTop:20}}>
            <span style={{fontSize:12,color:"hsl(var(--muted-foreground))"}}>Night</span>
            <button onClick={toggle} style={{
              width:46,height:24,borderRadius:12,
              background:mode==="light"?"hsl(var(--primary))":"hsl(var(--card))",
              border:".5px solid var(--border-glow)",cursor:"pointer",
              position:"relative" as const,transition:"background .25s"
            }}>
              <div style={{
                width:18,height:18,borderRadius:"50%",
                background:mode==="light"?"#fff":"hsl(var(--primary))",
            ...
            <span style={{fontSize:12,color:"hsl(var(--muted-foreground))"}}>Day</span>
          </div>
        </div>
            ...
            return(
              <div key={t.id}
                ...
                style={{
                  ...
                  background:td?.bgCard||"hsl(var(--card))",
                  border:`.5px solid ${isActive?td?.accent||"hsl(var(--primary))":td?.border||"hsl(var(--border))"}`,
                  boxShadow:isActive
                    ?`0 0 0 1px ${td?.accent||"hsl(var(--primary))"}, ${td?.shadowGlow||""}`
                ...
                <div style={{fontSize:13,fontWeight:700,color:td?.text||"hsl(var(--foreground))",
                  marginBottom:4,fontFamily:td?.fontDisplay||"inherit"}}>
                  {t.name}
                </div>
                <div style={{fontSize:11,color:td?.textMuted||"hsl(var(--muted-foreground))",
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap" as const}}>
                  {t.moods.map(m=>(
                    <span key={m} style={{
                      ...
                      color:td?.accentSoft||"hsl(var(--primary))",
                ...
                <div style={{marginTop:10,display:"flex",gap:4,alignItems:"center"}}>
                  <div style={{width:20,height:20,borderRadius:td?.radius||"12px",
                    ...
                    border:`.5px solid ${td?.accent||"hsl(var(--primary))"}30`}}/>
                  <span style={{fontSize:9,color:td?.textMuted||"hsl(var(--muted-foreground))",
                    fontFamily:td?.fontMono||"monospace"}}>r={td?.radius||"12px"}</span>
                </div>
                {isActive&&(
                  <div style={{marginTop:8,fontFamily:"var(--font-mono)",fontSize:9,
                    color:td?.accent||"hsl(var(--primary))",letterSpacing:".1em",
                    textTransform:"uppercase" as const,display:"flex",alignItems:"center",gap:4}}>
                    <span style={{width:5,height:5,borderRadius:"50%",
                      background:td?.accent||"hsl(var(--primary))",
                      boxShadow:`0 0 6px ${td?.accent||"hsl(var(--primary))"}`,
        ...
        <div style={{marginTop:32,padding:"24px",borderRadius:"var(--radius-lg)",
          background:"hsl(var(--card))",border:".5px solid var(--border)"}}>
          <div style={{fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"2px",
            textTransform:"uppercase" as const,color:"hsl(var(--muted-foreground))",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap" as const}}>
            ...
            <span className="empire-badge" style={{background:"hsl(var(--primary) / .2)",color:"hsl(var(--primary))",border:".5px solid var(--border-glow)"}}>BADGE</span>
