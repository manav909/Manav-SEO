        ...
        <div className="rounded-2xl border border-border/70 bg-card shadow-sm flex flex-col min-h-0 min-w-0">
          ...
          <div className="border-t border-border p-3 space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {conversation.trim().length > 20 && <button onClick={() => runStrategize(conversation, false)} disabled={analyzingIds.includes(selected?.id || "")} className="text-xs px-3 py-1.5 rounded-lg border border-primary/40 text-primary font-semibold disabled:opacity-50">{analyzingIds.includes(selected?.id || "") ? "Analysing…" : (strategy?.deal_state ? "Re-analyse" : "Analyse this chat")}</button>}
