...
const post2=(a:string,b:any={})=>fetch("/api/task-engine",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:a,...b})}).then(r=>r.json()).catch(()=>({}));

function DocGenerator({analysis,auditResult,assignments}:{analysis:any;auditResult:any;assignments:any[]}) {
  ...
  const S2:any = {
    ...
    typeBtn:(active:boolean)=>({
      ...
      background:active?"rgba(99,102,241,.12)":"hsl(var(--background))",
      outline:active?"1px solid rgba(99,102,241,.4)":"0.5px solid #1a1a3a",
  ...
  const generate = async () => {
    ...
    const r = await post2("generate_client_doc", {
      docType,
      conversationAnalysis: analysis,
      auditResult,
      leadInfo: { url: leadUrl, name: leadName, industry: leadIndustry },
    });
    setDoc((r as any).document || (r as any).error || "Generation failed — check API is running");
    setGenerating(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <div>
          <div style={S2.sec}>Document Type</div>
          <div style={{display:"flex",flexDirection:"column" as const,gap:5}}>
            {DOC_TYPES.map(dt=>(
              <button key={dt.id} style={S2.typeBtn(docType===dt.id)} onClick={()=>setDocType(dt.id)}>
        ...
        <div>
          <div style={S2.sec}>Client Context</div>
          <div style={{display:"flex",flexDirection:"column" as const,gap:8,marginBottom:14}}>
            <input style={S2.inp} placeholder="Client website (e.g. example.com)" value={leadUrl} onChange={e=>setLeadUrl(e.target.value)}/>
            <input style={S2.inp} placeholder="Client name or company" value={leadName} onChange={e=>setLeadName(e.target.value)}/>
            <input style={S2.inp} placeholder="Industry (e.g. dental clinic, e-commerce)" value={leadIndustry} onChange={e=>setLeadIndustry(e.target.value)}/>
          </div>

          {analysis?.main_need && (
            <div style={{...S2.card,borderColor:"rgba(16,185,129,.2)",marginBottom:10}}>
              <div style={{fontSize:10,color:"#10b981",fontWeight:700,marginBottom:6}}>FROM CONVERSATION ANALYSIS</div>
          ...
          {auditResult?.score !== undefined && (
            <div style={{...S2.card,borderColor:"rgba(99,102,241,.2)",marginBottom:10}}>
              <div style={{fontSize:10,color:"#a78bfa",fontWeight:700,marginBottom:6}}>FROM SITE AUDIT</div>
              <div style={{fontSize:11,color:"hsl(var(--muted-foreground))",lineHeight:1.5}}>
          ...
          <button style={{...S2.btn("#6366f1"),width:"100%",padding:"10px 0",fontSize:13}} onClick={generate} disabled={generating}>
        ...
        <div style={S2.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:6}}>
              <button style={S2.btn(copied?"#10b981":"#a78bfa")} onClick={copyDoc}>{copied?"✓ Copied!":"Copy"}</button>
              <button style={S2.btn("#6366f1")} onClick={download}>Download</button>
              <button style={S2.btn("#ef4444")} onClick={generate} disabled={generating}>Regenerate</button>
            </div>
          </div>
          <div style={{
            background:"rgba(0,0,0,.15)",borderRadius:8,padding:"12px 14px",
            fontSize:12,lineHeight:1.8,color:"#d0d0e8",
            whiteSpace:"pre-wrap" as const,maxHeight:400,overflowY:"auto" as const,
            border:"0.5px solid #1a1a3a",fontFamily:"inherit",
          }}>
            {doc}
          </div>
          <div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap" as const}}>
            {["proposal","pitch_email","followup_email"].filter(d=>d!==docType).slice(0,2).map(d=>(
              <button key={d} style={{...S2.btn("hsl(var(--muted-foreground))"),fontSize:10}}
                onClick={()=>{setDocType(d);setTimeout(generate,100);}}>
                Also generate: {DOC_TYPES.find(x=>x.id===d)?.label}
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={S.body}>
        ...
        {tab==="showcase"&&(
          <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[
                  ...
                  <button key={item.type} onClick={()=>window.location.href="/client-comms"}
                    style={{background:"hsl(var(--background))",border:"0.5px solid #1a1a3a",borderRadius:9,padding:"12px",
                      textAlign:"left" as const,cursor:"pointer"}}>
                    <div style={{fontSize:13,color:"hsl(var(--foreground))",fontWeight:600,marginBottom:3}}>{item.label}</div>
                    <div style={{fontSize:11,color:"hsl(var(--muted-foreground))"}}>{item.desc}</div>
            ...
            <div style={S.card}>
              <div style={{fontSize:12,fontWeight:700,marginBottom:8,color:"hsl(var(--foreground))"}}>📊 Technical Proof Points</div>
              <div style={{color:"hsl(var(--muted-foreground))",fontSize:11,marginBottom:10}}>Copy these technical credibility statements for Fiverr conversations</div>
              {[
                ...
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",
                  padding:"8px 10px",marginBottom:6,background:"hsl(var(--background))",borderRadius:8,border:"0.5px solid rgba(99,102,241,.12)"}}>
