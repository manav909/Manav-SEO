...
export default function HealthDashboard(){
  ...
  if(loading)return<div style={{...S.p,display:"flex",alignItems:"center",justifyContent:"center",color:"#4b4b6a"}}>Loading health data...</div>;
  return(<div style={S.p}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      <div><div style={{fontSize:22,fontWeight:700}}>❤️ Client Health</div><div style={{fontSize:13,color:"#8b8ba8",marginTop:4}}>Avg: {avg}/100 · {health.filter(h=>h.churn_risk==="high"||h.churn_risk==="critical").length} at risk</div></div>
      <button style={S.btn} onClick={calcAll} disabled={calc}>{calc?"Calculating...":"↻ Recalculate All"}</button>
    </div>
    {health.map((h:any)=><div key={h.project_id} style={{...S.c,borderColor:h.churn_risk==="high"||h.churn_risk==="critical"?`${rc[h.churn_risk]}40`:"#1e1e3a"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div><div style={{fontSize:15,fontWeight:700}}>{(h as any).projects?.name}</div><div style={{fontSize:12,color:"#8b8ba8",marginTop:2}}>{h.recommended_action}</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:28,fontWeight:700,fontFamily:"monospace",lineHeight:1,color:h.overall_score>=70?"#10b981":h.overall_score>=50?"#f59e0b":"#ef4444"}}>{h.overall_score}</div><div style={{fontSize:10,color:"#4b4b6a",textTransform:"uppercase"}}>Health</div></div>
      </div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
        <span style={{fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:20,background:`${rc[h.churn_risk]||"#4b4b6a"}15`,color:rc[h.churn_risk]||"#4b4b6a"}}>{h.churn_risk?.toUpperCase()} RISK</span>
        {h.upsell_signals?.length>0&&<span style={{fontSize:10,color:"#10b981"}}>💡 Upsell opportunity</span>}
      </div>
    </div>)}
    {!health.length&&<div style={{color:"#4b4b6a",textAlign:"center",padding:40,fontSize:14}}>No health data. Click Recalculate All to start.</div>}
  </div>);
}
