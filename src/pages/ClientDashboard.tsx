...
export default function ClientDashboard() {
  ...
  if(loading) return (
    <div style={{minHeight:"100vh",background:"hsl(var(--background))",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
      <PortalNav />
      ...
      <div style={{zIndex:1,textAlign:"center"}}>
        ...
        <div style={{color:"hsl(var(--muted-foreground))",fontSize:14}}>Loading your dashboard...</div>
      </div>
    </div>
  );

  return (
    <div className="empire-page" style={{minHeight:"100vh",background:"hsl(var(--background))",color:"hsl(var(--foreground))"}}>
        ...
        <div style={{background:"rgba(var(--bg-card),0.85)",backdropFilter:"blur(20px)",
          ...
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div>
              ...
              {selProject&&<div style={{fontSize:10,color:"hsl(var(--muted-foreground))",textTransform:"uppercase",letterSpacing:"1px"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            {projects.length>1&&(
              <select style={{background:"hsl(var(--card))",border:"0.5px solid var(--border)",
                borderRadius:8,color:"hsl(var(--foreground))",padding:"6px 12px",fontSize:12,cursor:"pointer"}}
          ...
          <div style={{padding:"32px 24px 24px",borderBottom:"0.5px solid var(--border)"}}>
            <div style={{maxWidth:900,margin:"0 auto"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:24}}>
                <div>
                  <div style={{fontSize:11,color:"hsl(var(--muted-foreground))",textTransform:"uppercase",letterSpacing:"1.5px",marginBottom:6}}>
                  ...
                  <div style={{fontSize:14,color:"hsl(var(--muted-foreground))",maxWidth:520,lineHeight:1.6}}>
                ...
                {health&&(
                  <div style={{textAlign:"center",flexShrink:0}}>
                    <div style={{position:"relative",display:"inline-block"}}>
                      <ProgressRing value={health.overall_score||0} size={72} stroke={5} color={health.overall_score>=70?"#10b981":health.overall_score>=50?"hsl(var(--primary))":"#ef4444"}/>
                      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
                        <div style={{fontSize:16,fontWeight:700,color:health.overall_score>=70?"#10b981":health.overall_score>=50?"hsl(var(--primary))":"#ef4444"}}>{health.overall_score}</div>
                      </div>
                    </div>
                    <div style={{fontSize:9,color:"hsl(var(--muted-foreground))",textTransform:"uppercase",letterSpacing:"1px",marginTop:4}}>Health</div>
                  </div>
                )}
              </div>
          ...
          {tab==="overview"&&(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div className="empire-card" style={{gridColumn:"1/-1"}}>
                <div style={{fontSize:11,color:"hsl(var(--muted-foreground))",fontWeight:600,letterSpacing:"1.2px",textTransform:"uppercase",marginBottom:14}}>
                  🎯 This Week's Focus
                </div>
                {brief?.priority_actions?.length>0
                  ? brief.priority_actions.slice(0,3).map((a:any,i:number)=>(
                    <div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:"0.5px solid var(--border)",alignItems:"flex-start"}}>
                      ...
                      <div>
                        <div style={{fontSize:13,fontWeight:600,color:"hsl(var(--foreground))",marginBottom:2}}>{a.action}</div>
                        <div style={{fontSize:12,color:"hsl(var(--muted-foreground))"}}>{a.why}</div>
                      </div>
                      <span style={{marginLeft:"auto",flexShrink:0,fontSize:9,fontWeight:700,padding:"3px 8px",borderRadius:20,
                        background:a.impact==="high"?"rgba(239,68,68,.1)":"hsl(var(--primary) / .2)",
                        color:a.impact==="high"?"#f87171":"hsl(var(--primary))"}}>
                        {(a.impact||"med").toUpperCase()}
                      </span>
                    </div>
                  ))
                  : <div style={{color:"hsl(var(--muted-foreground))",fontSize:13}}>Your strategy is being generated. Check back shortly.</div>
                }
              </div>

              {/* Wins */}
              <div className="empire-card">
                <div style={{fontSize:11,color:"hsl(var(--muted-foreground))",fontWeight:600,letterSpacing:"1.2px",textTransform:"uppercase",marginBottom:12}}>
                  ✅ Recent Wins
                </div>
                {brief?.wins?.length>0
                ...
                {!brief?.wins?.length&&!learnings.length&&<div style={{color:"hsl(var(--muted-foreground))",fontSize:12}}>Wins are being verified. Building your track record.</div>}
              </div>
                ...
                {alerts.slice(0,4).map((a:any)=>(
                  <div key={a.id} style={{display:"flex",gap:8,padding:"7px 0",borderBottom:"0.5px solid var(--border)",alignItems:"flex-start"}}>
                    <span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:20,flexShrink:0,marginTop:2,
                      background:a.severity==="warning"?"rgba(245,158,11,.1)":a.severity==="critical"?"rgba(239,68,68,.1)":"hsl(var(--primary) / .2)",
                      color:a.severity==="warning"?"#f59e0b":a.severity==="critical"?"#f87171":"hsl(var(--primary))"}}>
                      {a.severity?.toUpperCase()}
                    </span>
                    <div style={{fontSize:12,color:"hsl(var(--muted-foreground))",lineHeight:1.4}}>{a.title}</div>
                  </div>
                ))}
                {!alerts.length&&<div style={{color:"hsl(var(--muted-foreground))",fontSize:12}}>No alerts. Everything is running smoothly. ✓</div>}
              </div>
            </div>
          )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:16}}>
                {metrics.slice(0,8).reverse().map((m:any,i:number)=>(
                  <div className="empire-card" key={i} style={{padding:"12px 14px"}}>
                    ...
                    <div style={{fontSize:16,fontWeight:700,color:"hsl(var(--primary))",fontFamily:"monospace"}}>
                      {(m.organic_traffic||0).toLocaleString()}
                    </div>
                    <div style={{fontSize:9,color:"hsl(var(--muted-foreground))",textTransform:"uppercase"}}>Organic visits</div>
              ...
              <div className="empire-card">
                ...
                {learnings.map((l:any,i:number)=>(
                  <div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:"0.5px solid var(--border)",alignItems:"center"}}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:600,color:"hsl(var(--foreground))",marginBottom:3}}>{l.card_title}</div>
                      {l.what_worked?.slice(0,2).map((w:string,j:number)=>(
                        <div key={j} style={{fontSize:11,color:"#34d399"}}>✓ {w}</div>
                      ))}
                    </div>
                    <div style={{flexShrink:0,textAlign:"center"}}>
                      <ProgressRing value={l.confidence_score||0} size={40} stroke={3} color={l.confidence_score>=80?"#10b981":"hsl(var(--primary))"}/>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab==="reports"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div style={{fontSize:12,color:"hsl(var(--muted-foreground))"}}>Your campaign reports</div>
              ...
              {reports.map((r:any)=>(
                <div key={r.id} className="empire-card" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:600,color:"hsl(var(--foreground))",marginBottom:4}}>{r.title}</div>
                    <div style={{display:"flex",gap:8}}>
                      <span style={{fontSize:10,padding:"2px 8px",borderRadius:20,background:"hsl(var(--primary) / .2)",color:"hsl(var(--primary))"}}>{r.report_type}</span>
                      <span style={{fontSize:10,color:"hsl(var(--muted-foreground))"}}>{new Date(r.created_at).toLocaleDateString("en-GB")}</span>
                    </div>
                  </div>
                  <a href={`/reports/${r.token}`} target="_blank"
                    style={{background:"hsl(var(--primary) / .2)",border:"0.5px solid var(--border-glow)",
                      borderRadius:8,color:"hsl(var(--primary))",padding:"7px 14px",fontSize:11,
                      fontWeight:600,textDecoration:"none"}}>
                    View Report ↗
                  </a>
                </div>
              ))}
              {!reports.length&&<div className="empire-card" style={{textAlign:"center",padding:40,color:"hsl(var(--muted-foreground))"}}>
          ...
          {tab==="insights"&&(
            <div>
              <div className="empire-card" style={{marginBottom:14,borderColor:"var(--border-glow)"}}>
                ...
                <div style={{fontSize:13,color:"hsl(var(--muted-foreground))",lineHeight:1.7,marginBottom:12}}>
                ...
                {latest?.llm_visibility_score!=null&&(
                  <div style={{display:"flex",gap:12,alignItems:"center"}}>
                    <ProgressRing value={latest.llm_visibility_score} size={56} stroke={4} color={latest.llm_visibility_score>=60?"#10b981":"hsl(var(--primary))"}/>
                    <div>
                      <div style={{fontSize:20,fontWeight:700,color:"hsl(var(--primary))",fontFamily:"monospace"}}>{latest.llm_visibility_score}%</div>
                      <div style={{fontSize:11,color:"hsl(var(--muted-foreground))"}}>AI citation rate</div>
              ...
              {brief?.algorithm_watch?.length>0&&(
                <div className="empire-card">
                  ...
                  {brief.algorithm_watch.map((a:string,i:number)=>(
                    <div key={i} style={{padding:"7px 0",borderBottom:"0.5px solid var(--border)",fontSize:13,color:"hsl(var(--muted-foreground))"}}>
                      ◉ {a}
                    </div>
                  ))}
                </div>
