...
export default function StaffProfile() {
  ...
  if (!staff) return (
    <div style={{ minHeight:"100vh", background:"#06060e", color:"#e8e8f8",
      ...
      <div style={{ color:"hsl(var(--muted-foreground))" }}>Loading profile...</div>
    </div>
  );
  ...
  const S: any = {
    ...
    tab_: { padding:"9px 14px", fontSize:12, fontWeight:500, cursor:"pointer", border:"none",
      background:"transparent", color:"hsl(var(--muted-foreground))", borderBottom:"2px solid transparent" },
    ...
    sec: { fontSize:10, fontWeight:600, letterSpacing:1.2, textTransform:"uppercase" as const,
      color:"hsl(var(--muted-foreground))", marginBottom:8, marginTop:12 },
    ...
    <div style={S.root}>
      ...
      <div style={S.hdr}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <button onClick={()=>navigate(-1)} style={{ background:"none", border:"none", color:"hsl(var(--muted-foreground))", cursor:"pointer", fontSize:18 }}>←</button>
          <span style={{ fontSize:14, fontWeight:600 }}>Staff Profile</span>
        </div>
        {/* Staff switcher */}
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          <span style={{ fontSize:11, color:"hsl(var(--muted-foreground))" }}>View:</span>
          {allStaff.slice(0,6).map((s:any) => (
            <button key={s.id} onClick={() => navigate(`/profile/${s.id}`)}
              title={s.name}
              style={{ width:28, height:28, borderRadius:"50%", border:`1px solid ${s.id===staff.id?ROLE_META[s.role]?.color||"#6366f1":"hsl(var(--border))"}`,
      ...
      <div style={S.body}>
        ...
        <div style={{ ...S.card, background:`linear-gradient(135deg,${meta.bg},#0d0d1e)`,
          ...
          <div style={{ display:"flex", gap:20, alignItems:"flex-start" }}>
            ...
            {/* Info */}
            <div style={{ flex:1 }}>
              <div style={{ fontSize:20, fontWeight:700, color:"hsl(var(--foreground))", marginBottom:4 }}>{staff.name}</div>
              <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" as const, marginBottom:8 }}>
                ...
                {staff.email && <span style={{ fontSize:12, color:"hsl(var(--muted-foreground))" }}>{staff.email}</span>}
              ...
              <div style={{ display:"flex", gap:12, flexWrap:"wrap" as const }}>
                {[
                  ...
                  <div key={t.l} style={{ background:"rgba(0,0,0,.2)", borderRadius:8, padding:"8px 14px", minWidth:70 }}>
                    ...
                    <div style={{ fontSize:9, color:"hsl(var(--muted-foreground))", textTransform:"uppercase", marginTop:2 }}>{t.l}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              ...
              <div style={S.card}>
                ...
                {perf ? (
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
                      {[
                        ...
                        <div key={l} style={{ background:"#070710", borderRadius:8, padding:"10px 12px" }}>
                          ...
                          <div style={{ fontSize:9, color:"hsl(var(--muted-foreground))", textTransform:"uppercase" }}>{l}</div>
                        </div>
                      ))}
                    </div>
                    {/* Conversion bar */}
                    <div style={S.sec}>Conversion Rate</div>
                    <div style={{ height:8, background:"hsl(var(--border))", borderRadius:4, overflow:"hidden", marginBottom:4 }}>
                  ...
                  <div style={{ color:"hsl(var(--muted-foreground))", fontSize:12 }}>No performance data yet this month.</div>
                )}
              </div>
              ...
              <div style={S.card}>
                ...
                {staff.targets && Object.keys(staff.targets).length > 0 ? (
                  ...
                  <div style={{ color:"hsl(var(--muted-foreground))", fontSize:12 }}>No targets set. Edit in Settings.</div>
                )}
                ...
                {(!staff.permissions || Object.values(staff.permissions).filter(Boolean).length === 0) &&
                  <div style={{ color:"hsl(var(--muted-foreground))", fontSize:12 }}>Default role permissions active.</div>}
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              {assignments.slice(0,5).map((a:any)=>(
                <div key={a.id} style={{ display:"flex", gap:8, padding:"7px 0", borderBottom:"0.5px solid #111128", alignItems:"center" }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12, color:"hsl(var(--foreground))" }}>{a.prospects?.company||a.prospects?.url||"Lead"}</div>
                    <div style={{ fontSize:10, color:"hsl(var(--muted-foreground))" }}>{a.stage} · {a.source}</div>
                  </div>
                  {a.deal_value&&<div style={{ fontSize:11, color:"#10b981", fontFamily:"monospace" }}>${a.deal_value}</div>}
                </div>
              ))}
              {!assignments.length && <div style={{ color:"hsl(var(--muted-foreground))", fontSize:12 }}>No leads assigned yet.</div>}
            </div>
          </div>
        )}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  <div>
                    ...
                    <div style={{ display:"flex", gap:6, marginTop:4 }}>
                      ...
                      <span style={S.badge(a.priority==="hot"?"#ef4444":a.priority==="high"?"#f59e0b":"hsl(var(--muted-foreground))")}>
                        {a.priority?.toUpperCase()}
                      </span>
                      <span style={{ fontSize:10, color:"hsl(var(--muted-foreground))" }}>{a.source}</span>
                    </div>
                    {a.notes&&<div style={{ fontSize:11, color:"hsl(var(--muted-foreground))", marginTop:4 }}>{a.notes.slice(0,80)}</div>}
                  </div>
                  <div style={{ textAlign:"right" as const }}>
                    {a.deal_value&&<div style={{ fontSize:14, fontWeight:700, color:"#10b981", fontFamily:"monospace" }}>${a.deal_value}</div>}
                    {a.conversion_probability&&<div style={{ fontSize:10, color:"hsl(var(--muted-foreground))" }}>{a.conversion_probability}% likely</div>}
                  </div>
                </div>
                {a.conversion_probability && (
                  <div style={{ height:3, background:"hsl(var(--border))", borderRadius:2, marginTop:8, overflow:"hidden" }}>
        ...
        {tab==="chat" && (
          <div>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:12, overflowX:"auto" as const }}>
              {DEPTS.map(d => (
                <button key={d.key} style={{ ...S.btn(dept===d.key?"#a78bfa":"hsl(var(--muted-foreground))"),
                  borderColor:dept===d.key?"rgba(167,139,250,.4)":"hsl(var(--border))",
            ...
            <div style={{ ...S.card, padding:0, overflow:"hidden" }}>
              <div style={{ padding:"12px 14px", borderBottom:"0.5px solid #1a1a3a",
                ...
                <div style={{ fontSize:10, color:"hsl(var(--muted-foreground))" }}>{messages.length} messages</div>
              </div>
              <div style={{ height:360, overflowY:"auto" as const, padding:"12px 14px", display:"flex",
                  ...
                  return (
                    <div key={m.id} style={{ display:"flex", gap:8, alignItems:"flex-start",
                      ...
                      <div style={{ maxWidth:"70%" }}>
                        <div style={{ fontSize:10, color:"hsl(var(--muted-foreground))", marginBottom:3,
                        ...
                        <div style={{ background: isMe ? "rgba(99,102,241,.15)" : "var(--bg-card)",
                          border: `0.5px solid ${isMe?"rgba(99,102,241,.3)":"hsl(var(--border))"}`,
                ...
                {!messages.length && (
                  <div style={{ color:"hsl(var(--muted-foreground))", textAlign:"center" as const, padding:40, fontSize:13 }}>
          ...
          <div style={S.card}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
              {[["Name",staff.name],["Email",staff.email||"—"],["Role",ROLE_META[staff.role]?.label],["Timezone",staff.timezone]].map(([l,v])=>(
                <div key={l}>
                  <div style={{ fontSize:10, color:"hsl(var(--muted-foreground))", marginBottom:3 }}>{l}</div>
                  <div style={{ fontSize:13, color:"hsl(var(--foreground))", padding:"8px 12px", background:"#070710",
