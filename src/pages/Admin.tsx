...
export default function Admin() {
  ...
  const [metricsForm, setMetricsForm] = useState({
    organic_traffic: '', domain_rating: '', backlinks: '',
    ai_citations: '', conversions: '', revenue_impact: '',
  ...
  const logWork = async () => {
    if (!selectedProject || !logForm.hours || !logForm.title) {
      toast({ title: 'Missing fields', variant: 'destructive' });
      return;
    }
    setLoading(true);
  ...
  const fetchLiveMetrics = async () => {
    ...
    if (!proj?.url) {
      toast({ title: 'No URL found for this project', variant: 'destructive' });
      return;
    }
    setLoading(true);
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
                  ...
                  <div key={client.id} className="rounded-xl border border-border bg-background/40">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {client.company} · ${client.retainer_amount}/mo
                          </div>
                        </div>
                      </div>
                    ...
                    {expandedClient === client.id && (
                      <div className="px-4 pb-3 border-t border-border pt-3">
                        ...
                        <div className="text-xs font-semibold text-muted-foreground mb-1">Projects:</div>
        ...
        {tab === 'logs' && (
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl border border-border bg-card/60 p-6">
              <h2 className="font-bold text-base mb-4 flex items-center gap-2">
                <Clock className="h-4 w-4 text-primary" /> Log Work Done
              </h2>
              <div className="space-y-3">
                <div className="space-y-1">
                ...
                {[
                  ...
                  { key: 'description',    label: 'Details',         placeholder: 'Compressed images, deferred scripts...' },
                  { key: 'impact_metric',  label: 'Impact Metric',   placeholder: 'Page Load Speed' },
                  { key: 'impact_value',   label: 'Impact Value',    placeholder: 'Improved from 4.2s to 1.8s' },
                ].map(({ key, label, placeholder }) => (
                  <div key={key} className="space-y-1">
                    <Label className={labelClass}>{label}</Label>
                    <Input
            ...
            <div className="rounded-2xl border border-border bg-card/60 p-6">
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  <select
                    value={selectedProject}
                    onChange={e => { setSelectedProject(e.target.value); setFetchedMetrics(null); }}
                    className="w-full h-10 rounded-md border border-border bg-background/60 text-sm px-3"
                  >
                    <option value="">— Choose project —</option>
                    {projects.map(p => {
                ...
                {/* Auto-fetch button */}
                {selectedProject && projects.find(p => p.id === selectedProject)?.url && (
                  <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
                    <div className="text-xs font-mono text-primary uppercase tracking-wider mb-1">
                      Auto-fetch Real Data
                    </div>
                    <p className="text-xs text-muted-foreground mb-3">
                      Fetches live PageSpeed scores and Core Web Vitals directly from Google for{' '}
                ...
                {fetchedMetrics && (
                  <div className="rounded-xl border border-green-400/20 bg-green-400/5 p-4 space-y-3">
                    <div className="flex items-center gap-2 text-xs font-mono text-green-400 uppercase tracking-wider">
                      ...
                      Real Data — Google PageSpeed Insights
                    </div>
                    <div className="text-xs text-muted-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { label: 'Mobile Perf', value: fetchedMetrics.raw_scores?.mobile_performance },
                        { label: 'Desktop Perf', value: fetchedMetrics.raw_scores?.desktop_performance },
                        { label: 'SEO Score', value: fetchedMetrics.raw_scores?.seo },
                        { label: 'Accessibility', value: fetchedMetrics.raw_scores?.accessibility },
                      ].map(({ label, value }) => {
                    ...
                    <div>
                      <div className="text-xs font-mono text-muted-foreground uppercase tracking-wider mb-2">Core Web Vitals</div>
                      ...
                      <div>
                        ...
                        {fetchedMetrics.analysis.technical_issues.map((issue: string, i: number) => (
                          <div key={i} className="flex items-start gap-1.5 text-xs text-muted-foreground py-0.5">
                            <span className="text-red-400 shrink-0">x</span>{issue}
                          </div>
                        ))}
                      </div>
                    )}
                    {fetchedMetrics.analysis?.confirmed_positives?.length > 0 && (
                      <div>
                        ...
                        {fetchedMetrics.analysis.confirmed_positives.map((item: string, i: number) => (
                          <div key={i} className="flex items-start gap-1.5 text-xs text-muted-foreground py-0.5">
                            <span className="text-green-400 shrink-0">ok</span>{item}
                          </div>
                        ))}
                      </div>
                    )}
                    <div className="text-xs text-muted-foreground border-t border-border pt-2">
                      Note: Organic traffic, backlinks and DR require Google Search Console or Ahrefs — enter manually below
                    </div>
                  </div>
                )}
                ...
                <div className="rounded-xl border border-border bg-background/40 p-3">
                  ...
                  <div className="space-y-1 mt-3">
                    ...
                    <Input
                      ...
                      onChange={e => setMetricsForm(f => ({ ...f, recorded_at: e.target.value }))}
                      className={inputClass}
                    />
                  </div>
                </div>
