...
const ConfBadge = ({ confidence }: { confidence: number }) => {
  const cfg = confidence >= 80 ? { color: 'text-green-400', bg: 'bg-green-400/10 border-green-400/20', label: `${confidence}% verified` }
    : confidence >= 50 ? { color: 'text-yellow-400', bg: 'bg-yellow-400/10 border-yellow-400/20', label: `${confidence}% confidence` }
    : confidence > 0   ? { color: 'text-orange-400', bg: 'bg-orange-400/10 border-orange-400/20', label: `${confidence}% — estimated` }
...
const DataRow = ({ label, value, confidence, limitations }: { label: string; value: any; confidence: number; limitations?: string[] }) => {
...
export default function Audit() {
  ...
  const loadPastReports = async (pid: string) => {
    const { data } = await supabase
      .from('audit_reports')
      .select('id,created_at,overall_score,synced_to_metrics,url')
      .eq('project_id', pid)
  ...
  const saveReport = async () => {
    ...
    try {
      ...
      /* Also save to project.last_analysis */
  ...
  const syncToMetrics = async () => {
    ...
    try {
      ...
      /* Mark audit report as synced */
      if (savedId) {
        await supabase.from('audit_reports').update({ synced_to_metrics: true, synced_at: new Date().toISOString() }).eq('id', savedId);
      }
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div>
          <h1 className="text-xl font-bold mb-1 flex items-center gap-2">
            <Zap className="h-5 w-5 text-primary" />AI SEO Audit — 4-Agent Analysis System
          </h1>
          <p className="text-sm text-muted-foreground">
            Four independent agents analyse your site. Data points are cross-verified. Every metric shows its source and confidence level.
            Only verifiable data feeds the dashboard.
          </p>
        </div>

        {/* Confidence system explainer */}
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {selectedProjectId && savedId && (
                  <Button onClick={syncToMetrics} disabled={syncing} variant="outline" className="border-green-400/30 text-green-400 hover:bg-green-400/10">
            ...
            {s?.technical && (
              <SectionCard
                ...
                agent="Agent 1 — Sitemap, indexing, schema, canonical"
                ceiling={s.technical.ceiling}
                icon={Shield} color="#06b6d4"
                data={s.technical.data}>
            ...
            {s?.content && (
              <SectionCard
                title="Content & E-E-A-T Analyser"
                agent="Agent 2 — Fetches live HTML, AI analyses for quality signals"
                ceiling={s.content.ceiling}
            ...
            {s?.visibility && (
              <SectionCard
                ...
                agent="Agent 3 — Live Perplexity test, brand mention count, LLM readiness estimate"
                ceiling={s.visibility.ceiling}
                icon={Sparkles} color="#6366f1"
                data={s.visibility.data}>
            ...
            {s?.ranking && (
              <SectionCard
                title="Ranking & Competitive Intelligence"
                agent="Agent 4 — Live Google SERP per keyword, competitor domain checks"
                ceiling={s.ranking.ceiling}
              ...
              <div className="rounded-2xl border border-border bg-card/60 p-5 space-y-4">
                ...
                {syn.verified_strengths?.length > 0 && (
                  <div>
                    ...
                    {syn.verified_strengths.map((s: string, i: number) => (
