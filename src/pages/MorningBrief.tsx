...
export default function MorningBrief(){
  ...
  useEffect(()=>{load();},[]);
  const S:any={p:{minHeight:"100vh",background:"#070710",color:"#f0f0ff",padding:28,fontFamily:"-apple-system,system-ui,sans-serif"},c:{background:"#0d0d1a",border:"0.5px solid #1e1e3a",borderRadius:14,padding:20,marginBottom:14},sec:{fontSize:10,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase" as const,color:"#4b4b6a",marginBottom:10},item:{display:"flex",gap:10,padding:"9px 0",borderBottom:"0.5px solid #1e1e3a",fontSize:13,lineHeight:1.5 as const},btn:{background:"rgba(99,102,241,.15)",border:"0.5px solid rgba(99,102,241,.3)",borderRadius:8,color:"#a78bfa",padding:"8px 16px",fontSize:12,cursor:"pointer"}};
  if(loading)return<div style={{...S.p,display:"flex",alignItems:"center",justifyContent:"center",color:"#4b4b6a"}}>Loading brief...</div>;
  return(<div style={S.p}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
      <div><div style={{fontSize:11,color:"#4b4b6a",letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{new Date().toLocaleDateString("en-GB",{weekday:"long",day:"2-digit",month:"long",year:"numeric"})}</div><div style={{fontSize:22,fontWeight:700}}>🌅 Morning Brief</div>{brief?.headline&&<div style={{fontSize:14,color:"#8b8ba8",marginTop:6,maxWidth:600,lineHeight:1.6}}>{brief.headline}</div>}</div>
      <button style={S.btn} onClick={regen} disabled={gen}>{gen?"Generating...":"↻ Regenerate"}</button>
    </div>
    {brief?.priority_actions?.length>0&&<div style={S.c}><div style={S.sec}>🎯 Priority Actions</div>{brief.priority_actions.map((a:any,i:number)=><div key={i} style={S.item}><span style={{...imp[a.impact||"medium"],padding:"2px 8px",borderRadius:20,fontSize:10,fontWeight:600,flexShrink:0,alignSelf:"flex-start",marginTop:2}}>{(a.impact||"med").toUpperCase()}</span><div><div style={{fontWeight:600,marginBottom:2}}>{a.action}</div><div style={{fontSize:12,color:"#8b8ba8"}}>{a.why}</div></div></div>)}</div>}
    ...
    {!brief?.headline&&<div style={{...S.c,textAlign:"center",padding:40}}><div style={{color:"#4b4b6a",marginBottom:16}}>No brief generated yet.</div><button style={{...S.btn,padding:"12px 24px"}} onClick={regen}>Generate Today's Brief</button></div>}
  </div>);
}
