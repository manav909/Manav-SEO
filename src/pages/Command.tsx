/* ════════════════════════════════════════════════════════════════
   ...
   Phase 7 — S.E.A.S.O.N. Command Page.

   The single screen. One input. Pre-loaded briefing. Live activity.
   Honest gaps. Hollywood-grade motion.

   Layout:
     • Greeting block (types in)
     • Status sentence (types in, after greeting)
     • Big input box (focused, glowing)
     • Attention list (staggered fade-in below)
     • Quiet wins (staggered fade-in)
     • Behind-the-scenes drawer trigger (bottom-right, persistent)
═══════════════════════════════════════════════════════════════ */

import { useEffect, useRef, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  Sparkles, AlertCircle, CheckCircle2, Clock, Activity, ArrowRight,
  X, Send, RefreshCw, Database, Building2, ChevronDown,
} from 'lucide-react';
...
export default function Command() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!text || submitting || !selectedProjectId) return;
    setSubmitting(true);
    setResponse(null);
  ...
  /* Load activity when drawer opens */
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-background via-background to-card flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <Sparkles className="h-8 w-8 mx-auto mb-3 text-cyan-400/60" />
          <div className="text-lg font-bold text-foreground mb-1">S.E.A.S.O.N. needs a project to brief on</div>
          <div className="text-sm text-muted-foreground mb-4">Pick one from the project switcher up top.</div>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-card text-foreground relative overflow-hidden">
      {/* Ambient background — subtle animated radial gradient */}
      ...
      <div className="relative max-w-3xl mx-auto px-6 py-12">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }}
              ...
              {[
                ...
                <motion.button key={q} onClick={() => handleQuickAction(q)}
                  ...
                  className="text-xs px-3 py-1.5 rounded-full border border-border bg-card/40 text-muted-foreground hover:text-foreground hover:border-cyan-500/30 transition-colors">
                  {q}
                </motion.button>
              ))}
            </motion.div>
        ...
        {/* Attention + wins (hidden during response display) */}
        ...
        {error && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="mt-6 rounded-lg border border-red-500/30 bg-red-500/[0.06] p-3 text-xs text-red-400 flex items-start gap-2">
      ...
      {/* Behind-the-scenes drawer trigger */}
      <motion.button
        onClick={() => setActivityOpen(true)}
        initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 2.6 }}
        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
        className="fixed bottom-5 right-5 px-3 py-2 rounded-full border border-cyan-500/30 bg-card/80 backdrop-blur-sm text-cyan-400 hover:border-cyan-500/60 transition-colors text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-cyan-500/10">
        <motion.span
  ...
  return (
    <div className="space-y-3 pt-8">
      ...
      <div className="text-[11px] text-cyan-400/70 flex items-center gap-2 pt-2">
        ...
        <motion.span animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 1, repeat: Infinity, delay: 0.15 }}
          className="w-1 h-1 rounded-full bg-cyan-400" />
        <motion.span animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 1, repeat: Infinity, delay: 0.3 }}
          className="w-1 h-1 rounded-full bg-cyan-400" />
  ...
  return (
    <div className="pt-8 pb-2">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        {project?.project_name || briefing.project_name}
...
function AttentionPanel({ items }: { items: BriefingItemClient[] }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      {items.length === 0 ? (
        <div className="text-[12px] text-muted-foreground italic py-3">
          Nothing urgent. You can focus on building or just take a breath.
        </div>
      ) : (
...
function QuietWinsPanel({ items }: { items: BriefingItemClient[] }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      {items.length === 0 ? (
        <div className="text-[12px] text-muted-foreground italic py-3">
          No wins to report from the last 7 days yet.
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((item, i) => (
            <motion.div key={i}
              ...
              <div className="flex items-start gap-2">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                  transition={{ type: 'spring', delay: 2.2 + i * 0.08 }}>
...
function SeverityDot({ severity }: { severity: string }) {
  const color =
    ...
    severity === 'success'  ? 'bg-emerald-400' : 'bg-cyan-400';
  return (
    <motion.span
...
/* ─── Response panel (when user submits a command) ─────────── */
    ...
    <motion.div
      ...
      <div className="p-4 space-y-2">
        {response.chunks.map((c, i) => (
          <StreamingChunk key={i} chunk={c} delay={i * 0.15} />
        ))}
        {response.honest_note && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            transition={{ delay: response.chunks.length * 0.15 + 0.3 }}
...
function StreamingChunk({ chunk, delay }: { chunk: any; delay: number }) {
  ...
  if (chunk.kind === 'verify') {
    return (
      <motion.details initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: delay + 0.2 }}
        className="text-[10px]">
    ...
    <>
      ...
      <motion.aside
        ...
        <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-gradient-to-r from-cyan-500/[0.06] to-transparent">
          ...
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-muted/40 text-muted-foreground hover:text-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[9px] text-muted-foreground/60 mt-1">
                      {timeAgo(e.created_at)} · {e.source} · {e.event_type}
                    </div>
                  </div>
                </div>
              </motion.div>
