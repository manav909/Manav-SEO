  ...
  return (
    <div style={{ textAlign: "center" }}>
      <svg width={160} height={90} viewBox="0 0 160 90">
        ...
        {/* Track */}
        <path d="M 15 80 A 65 65 0 0 1 145 80" fill="none" stroke="hsl(var(--border))" strokeWidth={10} strokeLinecap="round"/>
        {/* Fill */}
...
function TZClock({ tz }: { tz: any }) {
  ...
  const c = tz.business_status === "business" ? "#10b981" : tz.business_status === "early" || tz.business_status === "evening" ? "#f59e0b" : "hsl(var(--muted-foreground))";
  return (
    <div style={{ background: "hsl(var(--card))", border: `0.5px solid ${c}30`, borderRadius: 9, padding: "8px 12px",
      display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <div>
        <div style={{ fontSize: 11, color: "hsl(var(--foreground))" }}>{tz.flag} {tz.region}</div>
        <div style={{ fontSize: 9, color: "hsl(var(--muted-foreground))" }}>{tz.day}</div>
      </div>
      <div>
...
export default function ClientComms() {
  ...
  const S: any = {
    ...
    card: { background: "hsl(var(--card))", border: "0.5px solid #1a1a3a", borderRadius: 12, padding: 16, marginBottom: 12 },
    ...
    sel: { background: "hsl(var(--card))", border: "0.5px solid #1a1a3a", borderRadius: 8,
    ...
    sec: { fontSize: 10, fontWeight: 600, letterSpacing: 1.2, textTransform: "uppercase" as const,
      color: "hsl(var(--muted-foreground))", marginBottom: 8, marginTop: 14 },
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.body}>
        ...
        {tab === "analyser" && (
          <div>
            <div style={{ marginBottom: 12, color: "hsl(var(--muted-foreground))", fontSize: 13 }}>
            ...
            <div style={S.card}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", flex: 1 }}>Paste conversation below — any language</div>
                {analysis && (
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                ...
                {convText && <button style={S.btn("hsl(var(--muted-foreground))")} onClick={() => { setConvText(""); setAnalysis(null); setResponses(null); }}>Clear</button>}
              </div>
            </div>
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    {analysis.emotional_subtext && (
                      <>
                        ...
                        <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", lineHeight: 1.5, fontStyle: "italic" }}>
                          "{analysis.emotional_subtext}"
                        </div>
                      </>
                    )}
                ...
                <div>
                  ...
                  {(analysis.cultural_context || analysis.geopolitical_notes) && (
                    <div style={{ ...S.card, borderColor: "rgba(6,182,212,.2)" }}>
                      ...
                      {analysis.geopolitical_notes && <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", lineHeight: 1.5, fontStyle: "italic" }}>{analysis.geopolitical_notes}</div>}
                    </div>
                  )}
                    ...
                    <div style={{ ...S.card, borderColor: "rgba(239,68,68,.2)" }}>
                      ...
                      {analysis.objections.map((obj: any, i: number) => (
                        <div key={i} style={{ display: "flex", gap: 10, padding: "7px 0", borderBottom: "0.5px solid #111128" }}>
                          <span style={{ ...S.badge(obj.severity === "dealbreaker" ? "#dc2626" : obj.severity === "strong" ? "#ef4444" : obj.severity === "moderate" ? "#f59e0b" : "hsl(var(--muted-foreground))") }}>
                            {obj.severity?.toUpperCase()}
                          </span>
                          <div>
                            <div style={{ fontSize: 11, color: "#a78bfa", fontWeight: 600, marginBottom: 2 }}>
                              {obj.type?.replace(/_/g, " ").toUpperCase()}
                              {obj.hidden && <span style={{ marginLeft: 6, fontSize: 9, color: "hsl(var(--muted-foreground))" }}>(implied)</span>}
                            </div>
            ...
            {responses?.responses?.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))", marginBottom: 10 }}>
                ...
                {responses.responses.map((resp: any, idx: number) => (
                  <div key={idx} style={{ ...S.card, borderColor: idx === 0 ? "rgba(99,102,241,.3)" : "hsl(var(--border))" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "hsl(var(--foreground))" }}>{resp.strategy}</div>
                        ...
                        {resp.when_to_use && <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>{resp.when_to_use}</div>}
                      </div>
                    ...
                    {resp.subject_line && (
                      <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginBottom: 8, padding: "4px 8px",
                ...
                {OBJECTION_TYPES.map(obj => (
                  <button key={obj.key}
                    ...
                    style={{ background: selObjType === obj.key ? "rgba(99,102,241,.2)" : "rgba(255,255,255,.03)",
                      border: `0.5px solid ${selObjType === obj.key ? "#6366f1" : "hsl(var(--border))"}`,
                      borderRadius: 9, padding: "10px 12px", fontSize: 12, color: selObjType === obj.key ? "#a78bfa" : "hsl(var(--muted-foreground))",
              ...
              {selObjType && (
                <>
                  <div style={{ display: "flex", gap: 10, marginBottom: 10, alignItems: "center" }}>
                    <div style={{ flex: 1, fontSize: 12, color: "hsl(var(--muted-foreground))" }}>
                ...
                <div style={{ ...S.card, borderColor: "rgba(16,185,129,.2)" }}>
                  ...
                  {objResult.generated.power_phrase && (
                    <div style={{ padding: "10px 14px", background: "rgba(99,102,241,.08)", borderRadius: 9,
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600, color: "hsl(var(--foreground))", lineHeight: 1.5 }}>
                        "{objResult.generated.power_phrase}"
                      </div>
                    </div>
                  )}
                ...
                {UPDATE_TYPES.map(u => (
                  <button key={u.key} onClick={() => setUpdateType(u.key)}
                    style={{ background: updateType === u.key ? "rgba(99,102,241,.2)" : "rgba(255,255,255,.03)",
                      border: `0.5px solid ${updateType === u.key ? "#6366f1" : "hsl(var(--border))"}`,
                    ...
                    <div style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", marginTop: 2 }}>{u.desc}</div>
                  </button>
                ))}
              </div>
              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>Language:</div>
        ...
        {tab === "presentations" && (
          <div>
                ...
                {PRES_TYPES.map(p => (
                  <button key={p.key} onClick={() => setPresType(p.key)}
                    style={{ background: presType === p.key ? "rgba(99,102,241,.2)" : "rgba(255,255,255,.03)",
                      border: `0.5px solid ${presType === p.key ? "#6366f1" : "hsl(var(--border))"}`,
                      borderRadius: 9, padding: "9px 14px", fontSize: 12,
                      color: presType === p.key ? "#a78bfa" : "hsl(var(--muted-foreground))", cursor: "pointer" }}>
                    {p.label}
                  </button>
                ))}
              </div>
            ...
            {presentations.length > 0 && (
              <div>
                <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", margin: "12px 0 8px" }}>Previous presentations</div>
                  ...
                  <div key={p.id} style={{ ...S.card, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                        ...
                        <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{p.viewed_count} views</span>
                        <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{new Date(p.created_at).toLocaleDateString("en-GB")}</span>
                      </div>
                    </div>
                    <a href={`/presentation/${p.token}`} target="_blank" style={{ ...S.btn("hsl(var(--muted-foreground))"), textDecoration: "none" }}>Open ↗</a>
                  </div>
                ))}
              </div>
            )}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {(["all","open","soon"] as const).map(f => (
                <button key={f} style={{ ...S.btn(f === tzFilter ? "#a78bfa" : "hsl(var(--muted-foreground))"),
                  borderColor: f === tzFilter ? "rgba(167,139,250,.4)" : "hsl(var(--border))" }}
                  onClick={() => setTzFilter(f)}>
                  {f === "all" ? "🌍 All" : f === "open" ? "🟢 Open Now" : "🟡 Soon Open"}
                </button>
              ))}
              <div style={{ marginLeft: "auto", fontSize: 12, color: "hsl(var(--muted-foreground))", alignSelf: "center" }}>
            ...
            {filteredTZ.length === 0 && (
              <div style={{ color: "hsl(var(--muted-foreground))", textAlign: "center", padding: 40 }}>
