...
export default function LLMVisibility(){
  ...
  const S:any={p:{minHeight:"100vh",background:"#070710",color:"#f0f0ff",padding:28,fontFamily:"-apple-system,system-ui,sans-serif"},c:{background:"#0d0d1a",border:"0.5px solid #1e1e3a",borderRadius:12,padding:18,marginBottom:10},sel:{background:"#0d0d1a",border:"0.5px solid #1e1e3a",borderRadius:8,color:"#f0f0ff",padding:"8px 14px",fontSize:13},btn:{background:"rgba(6,182,212,.12)",border:"0.5px solid rgba(6,182,212,.3)",borderRadius:8,color:"#06b6d4",padding:"9px 18px",fontSize:13,fontWeight:600,cursor:"pointer"}};
  if(loading)return<div style={{...S.p,display:"flex",alignItems:"center",justifyContent:"center",color:"#4b4b6a"}}>Loading...</div>;
  return(<div style={S.p}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      <div><div style={{fontSize:22,fontWeight:700}}>🤖 LLM Visibility</div><div style={{fontSize:13,color:"#8b8ba8",marginTop:4}}>How AI models see your clients</div></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:20}}>
      {[{v:`${rate}%`,l:"Citation Rate",c:rate>60?"#10b981":rate>30?"#f59e0b":"#ef4444"},{v:`${cited}/${cits.length}`,l:"Cited",c:"#06b6d4"},{v:cits.filter((c:any)=>c.sentiment==="positive").length,l:"Positive",c:"#10b981"},{v:cits.filter((c:any)=>!c.cited).length,l:"Gaps",c:"#f87171"}].map((t:any)=><div key={t.l} style={{background:"#0d0d1a",border:"0.5px solid #1e1e3a",borderRadius:12,padding:"16px 20px"}}><div style={{fontSize:26,fontWeight:700,color:t.c,fontFamily:"monospace",lineHeight:1,marginBottom:4}}>{t.v}</div><div style={{fontSize:11,color:"#4b4b6a",textTransform:"uppercase",letterSpacing:.8}}>{t.l}</div></div>)}
    </div>
    {cits.map((c:any,i:number)=><div key={i} style={{...S.c,background:c.cited?"rgba(16,185,129,.04)":"rgba(239,68,68,.03)",borderColor:c.cited?"rgba(16,185,129,.2)":"rgba(239,68,68,.15)"}}><div style={{display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:16,flexShrink:0}}>{c.cited?"✅":"❌"}</span><div><div style={{fontSize:13,fontWeight:600,marginBottom:3}}>{c.query?.slice(0,70)}</div><div style={{fontSize:12,color:"#8b8ba8",lineHeight:1.4}}>{c.improvement_hint?.slice(0,120)}</div></div></div></div>)}
    {!cits.length&&<div style={{color:"#4b4b6a",textAlign:"center",padding:40,fontSize:14}}>No checks yet. Run a visibility check above.</div>}
  </div>);
}
