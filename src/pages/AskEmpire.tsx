...
export default function AskEmpire(){
  ...
  const S:any={
    ...
    bubble:(role:string)=>({
      ...
      border:`0.5px solid ${role==="user"?"transparent":"var(--border)"}`,
      color:role==="user"?"#fff":"var(--text)",fontSize:14,lineHeight:1.7,
      alignSelf:role==="user"?"flex-end":"flex-start" as const,
      boxShadow:role==="user"?`0 4px 20px var(--accent-glow)`:"0 2px 12px rgba(0,0,0,.2)",
    }),
    inp:{flex:1,background:"var(--bg-card)",border:"0.5px solid var(--border)",
      ...
      fontFamily:"inherit"},
    btn:{background:"var(--accent)",border:"none",borderRadius:10,color:"#fff",
      padding:"12px 20px",fontSize:14,fontWeight:600,cursor:"pointer",
      transition:"all .2s",flexShrink:0,
      boxShadow:"0 4px 16px var(--accent-glow)"},
    suggestion:{background:"var(--bg-card)",border:"0.5px solid var(--border)",
      borderRadius:20,padding:"7px 14px",fontSize:12,color:"var(--text-sub)",
      ...
      <div style={{position:"relative",zIndex:1,height:"100vh",display:"flex",flexDirection:"column" as const}}>
        <div style={S.hdr}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:32,height:32,borderRadius:"50%",background:"var(--accent-glow)",
          ...
          {messages.length===0?(
            <div style={{maxWidth:720,margin:"0 auto"}}>
              <div style={{textAlign:"center" as const,padding:"48px 0 32px"}}>
                ...
                <div style={{fontSize:14,color:"var(--text-sub)",maxWidth:400,
              ...
              <div style={{display:"flex",flexWrap:"wrap" as const,gap:8,
                ...
                {SUGGESTIONS.map((s,i)=>(
                  <button key={i} style={S.suggestion} onClick={()=>ask(s)}
                    onMouseEnter={e=>{(e.target as any).style.borderColor="var(--border-glow)";(e.target as any).style.color="var(--text)"}}
                    onMouseLeave={e=>{(e.target as any).style.borderColor="var(--border)";(e.target as any).style.color="var(--text-sub)"}}>
                    {s}
                  </button>
                ))}
              </div>
              ...
              {messages.map((m,i)=>(
                <div key={i} style={{display:"flex",
                  ...
                  <div style={S.bubble(m.role)}>
                    ...
                    <div style={{fontSize:10,color:m.role==="user"?"rgba(255,255,255,.5)":"var(--text-muted)",
        ...
        <div style={{padding:"16px 24px",borderTop:"0.5px solid var(--border)",
          ...
          <div style={{maxWidth:720,margin:"6px auto 0",fontSize:11,color:"var(--text-muted)",textAlign:"center" as const}}>
