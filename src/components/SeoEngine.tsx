  ...
  try {
    ...
    const recentAuditSummaries = recentAudits
      .map((a: any) => {
        ...
        const snippetLen = mode === 'deep' ? 600 : 250;
        const snippet = first.slice(0, snippetLen).replace(/\n+/g, ' ').trim();
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`relative text-left rounded-xl border p-4 transition-all duration-200 ${
                mode === 'standard'
                  ? 'border-primary bg-primary/10 shadow-[0_0_20px_-8px_hsl(var(--primary)/0.5)]'
                  : 'border-border bg-secondary/40 hover:border-primary/30'
              }`}
            >
              <div className="flex items-center gap-2 mb-1.5">
                <div className={`h-2 w-2 rounded-full ${mode === 'standard' ? 'bg-primary' : 'bg-muted-foreground/30'}`} />
                <span className="text-sm font-semibold">Standard</span>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Fast &amp; complete. Concise sections, tight bullets. Best for quick wins and regular check-ins.
              </p>
              <div className="mt-2 text-xs font-mono text-muted-foreground">~2–4 min · 8k output</div>
            </button>
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                <div className={`h-2 w-2 rounded-full ${mode === 'deep' ? 'bg-primary' : 'bg-muted-foreground/30'}`} />
                <span className="text-sm font-semibold">Deep &amp; Detailed</span>
                <span className="text-xs bg-primary/10 text-primary border border-primary/20 rounded-full px-1.5 py-0.5 font-mono">Pro</span>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Exhaustive analysis. Full context, detailed examples, numbered action items, max depth per section.
              </p>
              <div className="mt-2 text-xs font-mono text-muted-foreground">~6–10 min · 16k output</div>
            </button>
          </div>
        </div>
        ...
        >
          {loading ? (
            ...
            <span className="flex items-center gap-2">
              ...
              {projectId ? `Run ${mode === 'deep' ? 'Deep ' : ''}Project Audit` : `Generate ${mode === 'deep' ? 'Deep ' : ''}Strategy`}
