/**
 * ◈ MANAV BRAIN — GOD MODE v3
 * Complete rewrite — clean, no patches, compiles first time.
 */
...
interface Reminder { id: string; text: string; deskId?: string; }
...
const PAGE_SUGGESTIONS: Record<string, string[]> = {
  ...
  '/dashboard':      ["How are my metrics trending?","What's the biggest opportunity right now?","What should I focus on this week?"],
  '/data-room':      ["What context am I missing?","Review my project brief and suggest improvements","What keywords should I be targeting?"],
...
const shouldIgnore = (msg: string) => !msg || IGNORE_PATTERNS.some(p => msg.includes(p));
const isRealError  = (msg: string) => {
  if (!msg || msg.length < 10 || shouldIgnore(msg)) return false;
  return REAL_ERROR_PATTERNS.some(p => msg.includes(p));
};
const parseActions = (text: string): ParsedAction[] => {
...
const stripActions = (t: string) =>
  t.replace(/⟦ACTION⟧[\s\S]*?⟦\/ACTION⟧/g,'').replace(/\n{3,}/g,'\n\n').trim();
...
function MsgBubble({ msg, onAction, onSaveToDesk }: {
  msg: BrainMsg;
  onAction: (a: ParsedAction) => void;
  onSaveToDesk?: () => void;
}) {
  ...
  if (isSys) return (
    <div style={{textAlign:'center',padding:'2px 0'}}>
      <span style={{fontSize:7,fontFamily:'monospace',color:'rgba(255,255,255,0.15)',letterSpacing:'0.1em'}}>{msg.content}</span>
    </div>
  );
  if (isAlert) return (
...
export default function ManavBrainAssistant() {
  ...
  /* ─────────────────────────────────────────────────────────
     brainFetch — bypasses fetch override to avoid self-monitoring
  ───────────────────────────────────────────────────────── */
  ...
  const readFileAsBase64 = useCallback((file: File): Promise<{data:string;mediaType:string;name:string;type:string}> =>
    new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        resolve({ data:b64, mediaType:mt, name:file.name, type:mt.startsWith('image/')? 'image':'document' });
      };
  ...
  /* ── Auto-select project from localStorage ── */
  useEffect(() => {
  ...
  /* ── Sync project across tabs ── */
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'seo_season_proj' && e.newValue && e.newValue !== selProj) {
        setSelProj(e.newValue); setAutoProj(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const hi  = h < 12 ? 'Good morning! ☀️' : h < 17 ? 'Hey, good afternoon!' : 'Good evening!';
    setTimeout(() => {
      setOpen(true); setTab('chat');
      setMsgs(ms => [...ms, { id:uid(), role:'brain' as MsgRole,
        content: hi + " I've been keeping an eye on things. Let me pull up what's most important for you right now...",
        ts:new Date() }]);
      setTimeout(() => sendMsgInternal(
        'Give me a concise good ' + tod + ' briefing. 3-4 short bullets: (1) most urgent thing today, (2) what is looking good, (3) one thing to do in the next hour. Warm and direct.',
        true), 800);
    }, 2500);
  ...
  useEffect(() => {
    ...
    const learningsMsg = learnings.length > 0
      ? 'You have ' + learnings.length + ' active learnings. Want me to apply the best ones to your canvas?'
      : 'As we work together I will build up your learning library automatically.';
    const deskMsg = deskItems.length > 0
      ? 'Your desk has ' + deskItems.length + ' saved items. Want me to summarise the most important ones?'
      : 'Save anything useful here and I will remind you about it when it matters.';
  ...
  useEffect(() => {
    ...
    const due = deskItems.filter(item => {
      ...
      return age > TWO_DAYS && !sessionStorage.getItem('reminded_' + item.id);
    }).slice(0, 2);
    if (due.length === 0) return;
    setReminders(due.map(item => ({
      id:   item.id,
      text: Math.floor((now - new Date(item.created_at).getTime()) / (24*60*60*1000)) + ' days ago you saved: "' + item.title + '"',
    })));
  }, [deskItems, selProj]);
  ...
  useEffect(() => {
    ...
    if (week1.length > 0) insights.push('Week 1 has ' + week1.length + ' task' + (week1.length>1?'s':'') + ' still to go — want to tackle one now?');
    if (insights.length === 0) return;
    const key = 'smart_scan_' + selProj + '_' + new Date().toDateString();
    if (sessionStorage.getItem(key)) return;
    sessionStorage.setItem(key, '1');
    setTimeout(() => setMsgs(ms => [...ms, { id:uid(), role:'brain' as MsgRole,
    ...
    window.fetch = async function monitoredFetch(...args: Parameters<typeof fetch>) {
      ...
      try {
        ...
        if (url.includes('/api/') && res.status >= 500) {
          let body = '';
          try { body = (await res.clone().text()).slice(0,400); } catch (_e) { /* ok */ }
          window.dispatchEvent(new CustomEvent('manav-brain-error', {
            detail:{type:'api_error',url,status:res.status,message:'HTTP '+res.status+' from '+(url.split('/').pop()||url),body},
          }));
        }
        return res;
      } catch (err: any) {
        if (url.includes('/api/')) window.dispatchEvent(new CustomEvent('manav-brain-error',{detail:{type:'network_error',url,message:err.message||'Network request failed'}}));
        throw err;
      }
    };
    const onJSError   = (e: ErrorEvent) => { if (!isRealError(e.message)) return; window.dispatchEvent(new CustomEvent('manav-brain-error',{detail:{type:'js_error',message:e.message,stack:e.error?.stack?.slice(0,600),url:e.filename}})); };
    const onRejection = (e: PromiseRejectionEvent) => { const msg=e.reason?.message||String(e.reason); if (!isRealError(msg)) return; window.dispatchEvent(new CustomEvent('manav-brain-error',{detail:{type:'promise_rejection',message:msg,stack:e.reason?.stack?.slice(0,400)}})); };
    const origConsole = console.error.bind(console);
    console.error = (...args: any[]) => { origConsole(...args); const msg=args.map(a=>typeof a==='string'?a:(a?.message||'')).join(' '); if (isRealError(msg)&&msg.length>30) window.dispatchEvent(new CustomEvent('manav-brain-error',{detail:{type:'console_error',message:msg.slice(0,300)}})); };
    const onBrainEvent = (e: Event) => { errHandlerRef.current?.((e as CustomEvent).detail); };
  ...
  useEffect(() => {
    setTimeout(async () => {
      try {
        const { error } = await supabase.from('brain_learnings').select('status').limit(1);
        setSchemaOk(!error || !error.message.includes('column "status"'));
      } catch (_e) { /* silent */ }
    }, 3000);
  }, []); // eslint-disable-line
  ...
  const handleSystemError = useCallback((detail: any) => {
    ...
    setTimeout(() => { healCooldown.current = false; }, 60000);
  ...
  const triggerHealing = useCallback((err: SysError) => {
    setHealth('healing'); setScanLine(true);
    setTimeout(() => setScanLine(false), 2500);
    setMsgs(ms => [...ms, { id:uid(), role:'alert' as MsgRole, content:'ANOMALY — '+err.type.replace(/_/g,' ').toUpperCase()+' DETECTED ON '+err.page, ts:new Date() }]);
    const prompt = ['Error Type: '+err.type,'Message: '+err.message,err.stack?'Stack:\n'+err.stack:'',err.url?'File: '+err.url:'',err.status?'HTTP: '+err.status:'',err.body?'Body: '+err.body:'','Diagnose this completely. What is the exact root cause? Provide the exact fix — specific code change, SQL, or config. Be direct.'].filter(Boolean).join('\n');
    void sendMsgInternal(prompt, true, err);
  ...
  const logSystemError = useCallback(async (err: SysError) => {
    try {
      const row: any = { project_id:selProj||null, card_type:err.type.includes('api')?'technical':'general', card_title:'System Error: '+err.type+' — '+err.message.slice(0,50), what_worked:[], what_missed:[err.message,err.body||err.stack?.slice(0,200)||'No details'].filter(Boolean), improvement:'Fix: '+err.type+' on '+err.page+' — pending', context_summary:err.type+' at '+err.ts.toISOString()+' on '+err.page, tags:['system_error_log',err.type], source:'system_error_log', applied_count:0, updated_at:new Date().toISOString() };
      try { await supabase.from('brain_learnings').insert({...row,status:'active',auto_captured:true,confidence_score:98}); } catch (_e) { await supabase.from('brain_learnings').insert(row); }
    } catch (_e) { /* silent */ }
  }, [selProj]);

  const logHealAction = useCallback(async (errMsg: string, fix: string, actions: string[]) => {
    try {
      const row: any = { project_id:selProj||null, card_type:'technical', card_title:'Brain Healed: '+errMsg.slice(0,50), what_worked:actions, what_missed:[], improvement:fix.slice(0,400), context_summary:'Self-healing at '+new Date().toISOString(), tags:['brain_heal_log','self_healing'], source:'brain_heal_log', applied_count:0, updated_at:new Date().toISOString() };
      try { await supabase.from('brain_learnings').insert({...row,status:'active',auto_captured:true,confidence_score:95}); } catch (_e) { await supabase.from('brain_learnings').insert(row); }
    } catch (_e) { /* silent */ }
  }, [selProj]);
  ...
  const logConversation = useCallback(async (userMsg: string, brainResp: string, actions: ParsedAction[]) => {
    if (!selProj) return;
    try {
      const row: any = { project_id:selProj, card_type:'general', card_title:'Brain Chat: '+userMsg.slice(0,55), what_worked:actions.map(a=>a.label||a.type).filter(Boolean), what_missed:[], improvement:brainResp.slice(0,400), context_summary:'Manav Brain conversation — '+new Date().toLocaleDateString(), tags:['brain_assistant_log',...new Set(actions.map(a=>a.type))], source:'brain_assistant_log', applied_count:0, updated_at:new Date().toISOString() };
      try { await supabase.from('brain_learnings').insert({...row,status:'active',auto_captured:true,confidence_score:90}); } catch (_e) { await supabase.from('brain_learnings').insert(row); }
    } catch (_e) { /* silent */ }
  }, [selProj]);
  ...
  const loadContext = useCallback(async () => {
    ...
    try {
      ...
      if (fullD.success) {
        ...
        try { const blocks=fc.canvas||[]; setCanvasBlocks(Array.isArray(blocks)?blocks:[]); } catch (_e) { setCanvasBlocks([]); }
      }
    } catch (_e) { /* silent */ }
  }, [selProj, brainFetch]);
      ...
      switch (action.type) {
        ...
        case 'generate_report': {
          try {
            ...
            const doc=new jsPDF(); const proj=projects.find(p=>p.id===selProj); const date=new Date().toLocaleDateString();
            doc.setFillColor(3,7,18); doc.rect(0,0,210,297,'F'); doc.setTextColor(165,180,252); doc.setFontSize(20); doc.text(action.title||'SEO Report',20,25); doc.setTextColor(255,255,255); doc.setFontSize(10); doc.text('Project: '+(proj?.name||'Unknown')+' | '+date,20,35); doc.setDrawColor(99,102,241); doc.line(20,40,190,40);
            let y=52; (action.sections||[]).forEach((s:any)=>{if(y>260){doc.addPage();y=20;} doc.setTextColor(165,180,252);doc.setFontSize(13);doc.text(s.heading||'',20,y);y+=8;doc.setTextColor(200,200,200);doc.setFontSize(10);const ls=doc.splitTextToSize(s.content||'',170);doc.text(ls,20,y);y+=ls.length*6+8;});
            doc.save(((action.title||'report').replace(/\s+/g,'-').toLowerCase())+'.pdf'); upd('done','Report downloaded: '+(action.title||'SEO Report'));
          } catch (err: any) { upd('error','Report failed: '+err.message); }
          break;
        }

        case 'generate_chart': { const chartId='chart_'+Date.now(); setInlineCharts(prev=>({...prev,[chartId]:{...action,id:chartId}})); setMsgs(ms=>ms.map(m=>m.id===msgId?{...m,chartId}:m)); upd('done','Chart rendered.'); break; }
        ...
        case 'save_to_desk': {
          ...
          upd('done','Saved to Desk: "' + (action.title||'Brain Note') + '" — I will remind you about it if it needs attention.');
          break;
        }

        case 'fetch_url': { const res=await brainFetch('/api/intelligence',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'deep_dive',question:'Analyse this URL for SEO issues: '+action.url,checkUrl:action.url,projectSummary:'',role:'senior_seo'})}); if(!res.body){upd('error','Failed to fetch');break;} const reader=res.body.getReader();const dec=new TextDecoder();let t=''; while(true){const{done,value}=await reader.read();if(done)break;t+=dec.decode(value);} upd('done',t.slice(0,400)); break; }
  ...
  const sendMsgInternal = useCallback(async (text: string, isAuto=false, sourceErr?: SysError) => {
    ...
    if (!isAuto) { setInput(''); if(codeSnippet){setCodeSnippet('');setShowCodePaste(false);} if(attachments.length)setAttachments([]); }
    if (!isAuto) setMsgs(ms=>[...ms,{id:uid(),role:'user',content:text,ts:new Date()}]);
    const brainId = uid();
    setMsgs(ms=>[...ms,{id:brainId,role:'brain',content:'',ts:new Date()}]);
    const history = msgs.filter(m=>m.role==='user'||m.role==='brain').slice(-8).map(m=>({role:m.role==='user'?'user':'assistant',content:stripActions(m.content).slice(0,250)}));
    ...
    try {
      ...
      if(sourceErr){const fix=stripActions(full).slice(0,400);setSysErrors(errs=>errs.map(e=>e.id===sourceErr.id?{...e,healed:true,healNote:fix}:e));setAlerts(n=>Math.max(0,n-1));setHealth(h=>h==='critical'?'healthy':h);void logHealAction(sourceErr.message,fix,actions.map(a=>a.label||a.type));}
        ...
        <div style={{position:'fixed',bottom:24,right:24,zIndex:9990,width:panelW,height:panelH,borderRadius:20,overflow:'hidden',background:'#030712',border:'1px solid '+(alertCount>0?'rgba(239,68,68,0.3)':'rgba(99,102,241,0.22)'),boxShadow:'0 0 80px '+(alertCount>0?'rgba(239,68,68,0.1)':'rgba(99,102,241,0.1)')+',0 20px 60px rgba(0,0,0,0.85)',display:'flex',flexDirection:'column',transition:'all 0.3s cubic-bezier(0.4,0,0.2,1)'}}>
          ...
          <div style={{position:'relative',zIndex:2,display:'flex',alignItems:'center',gap:8,padding:'10px 14px',background:'rgba(0,0,0,0.45)',borderBottom:'1px solid '+(alertCount>0?'rgba(239,68,68,0.12)':'rgba(99,102,241,0.12)'),backdropFilter:'blur(20px)'}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:7,fontFamily:'monospace',color:'rgba(255,255,255,0.2)',letterSpacing:'0.06em'}}>{loading?'Thinking...':health==='healing'?'On it! Self-healing...':health==='scanning'?'Checking everything...':schemaOk===false?'Quick setup needed':alertCount>0?alertCount+' thing'+(alertCount>1?'s':'')+' to look at':hc.label}</div>
            </div>
            ...
            <button onClick={()=>navigate('/desk')} style={{background:deskItems.length>0?'rgba(16,185,129,0.08)':'none',border:'none',cursor:'pointer',color:deskItems.length>0?'rgba(52,211,153,0.7)':'rgba(255,255,255,0.18)',padding:3,display:'flex',alignItems:'center',gap:2,fontSize:7,fontFamily:'monospace'}}>
          ...
          <div style={{position:'relative',zIndex:2,display:'flex',background:'rgba(0,0,0,0.25)',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
            {([{id:'chat',label:'CHAT',badge:null},{id:'system',label:'SYSTEM',badge:unresolvedErrors.length>0?unresolvedErrors.length:null,color:'#ef4444'},{id:'log',label:'LOG',badge:sysErrors.length>0?sysErrors.length:null,color:'#6366f1'}] as const).map(t=>(
                ...
                {msgs.map(m=>(
                  <MsgBubble key={m.id} msg={m}
                    onSaveToDesk={m.role==='brain' ? async()=>{
                      const r=await brainFetch('/api/task-engine',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'desk_save',project_id:selProj||null,title:'Brain '+m.ts.toLocaleDateString()+' '+m.ts.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),content_type:'text',content:stripActions(m.content),source:'brain_chat'})});
              ...
              <div style={{position:'relative',zIndex:2,padding:'10px 14px 14px',borderTop:'1px solid rgba(255,255,255,0.04)',background:'rgba(0,0,0,0.18)'}}>
                ...
                {showCodePaste && (
                  <div style={{marginBottom:8,background:'rgba(6,182,212,0.04)',border:'1px solid rgba(6,182,212,0.15)',borderRadius:8,padding:8}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                      ...
                      <button onClick={()=>{setCodeSnippet('');setShowCodePaste(false);}} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.3)',fontSize:8}}>✕</button>
                    </div>
                    <textarea value={codeSnippet} onChange={e=>setCodeSnippet(e.target.value)} rows={5} placeholder="Paste any file content here — TypeScript, TSX, SQL, JSON..." style={{width:'100%',background:'rgba(0,0,0,0.4)',border:'1px solid rgba(6,182,212,0.1)',borderRadius:6,padding:'6px 8px',fontSize:9,color:'rgba(255,255,255,0.6)',outline:'none',resize:'vertical',fontFamily:'monospace',boxSizing:'border-box'}}/>
                  </div>
                )}
                ...
                <div style={{marginTop:5,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  ...
                  <div style={{display:'flex',gap:8,alignItems:'center'}}>
                    <button onClick={()=>fileInputRef.current?.click()} style={{background:'none',border:'none',cursor:'pointer',fontSize:7,fontFamily:'monospace',color:'rgba(255,255,255,0.25)',padding:'1px 4px'}}>📎 {attachments.length>0?attachments.length+' file(s)':'FILE'}</button>
                    <button onClick={()=>setShowCodePaste(v=>!v)} style={{background:'none',border:'none',cursor:'pointer',fontSize:7,fontFamily:'monospace',color:showCodePaste?'rgba(6,182,212,0.8)':'rgba(255,255,255,0.25)',padding:'1px 4px'}}>CODE{codeSnippet?' ✓':''}</button>
          ...
          {tab==='system' && (
            <div style={{flex:1,overflow:'auto',padding:'14px',position:'relative',zIndex:2,display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{background:hc.color+'0c',border:'1px solid '+hc.color+'28',borderRadius:12,padding:'10px 14px',display:'flex',alignItems:'center',gap:10}}>
                ...
                <button onClick={runHealthScan} style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:7,padding:'4px 10px',color:'rgba(255,255,255,0.4)',fontSize:8,fontFamily:'monospace',cursor:'pointer',display:'flex',alignItems:'center',gap:4}}><RefreshCw size={8}/> SCAN NOW</button>
              </div>
              <div>
                <div style={{fontSize:8,fontFamily:'monospace',color:'rgba(255,255,255,0.22)',letterSpacing:'0.12em',marginBottom:8}}>API STATUS</div>
                {[{name:'task-engine',label:'Task Engine',icon:Zap},{name:'algorithm-intel',label:'Algorithm Intel',icon:Cpu},{name:'supabase',label:'Supabase DB',icon:Database}].map(ep=>{
                  const s=apiStatus[ep.name]||'checking'; const c=s==='ok'?'#10b981':s==='error'?'#ef4444':'#6366f1';
                  return (
                    <div key={ep.name} style={{display:'flex',alignItems:'center',gap:8,background:'rgba(255,255,255,0.02)',border:'1px solid '+c+'18',borderRadius:8,padding:'7px 10px',marginBottom:5}}>
                      <ep.icon size={10} style={{color:c,flexShrink:0}}/><span style={{fontSize:10,color:'rgba(255,255,255,0.45)',flex:1,fontFamily:'monospace'}}>{ep.label}</span>
                      {s==='error'&&<button onClick={()=>{setTab('chat');sendMsgInternal('Diagnose why '+ep.label+' is offline. Give me the exact fix.',true);}} style={{background:'rgba(239,68,68,0.08)'
