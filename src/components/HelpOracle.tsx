...
const PAGES: Record<string, {
  ...
  '/pm': {
    ...
    sections: [
      ...
      { title: 'Brand Studio — Client Collaboration (H.6a)', icon: '🤝', items: [
        'H.6a adds named-identity client collaboration on top of the legacy bare-token portal. Two surfaces coexist: /c/:token for one-off shares (no identity, read-only-ish) and /c/workspace for invited users with named identity (comments, approvals, sharing, intake forms, file uploads).',
        'Magic-link auth: in Brand Studio → Client Access → Client Users, click Invite. Enter email + role (6 roles: client_executive, client_marketing, client_legal, client_designer, client_internal, client_press_contact). System generates an invite token; copy the link directly or email it. The recipient opens it, enters their display name, and gets a persistent 90-day session in their browser. No password.',
        'Six roles drive what intake forms they see, what notifications they receive, and default audience matching. Roles are scoped per-project — the same human at a multi-project client needs separate invites per project. Discipline: project-level data isolation is airtight.',
        'Per-document share grants (document_share_grants table): three access levels — view, comment, approve. A client with comment+ can grant peers access. Hierarchy is enforced server-side: you cannot grant access higher than your own. PM (staff) can always grant any level and can revoke anyone\'s grant.',
        'Investor-grade documents (client_resharable=false) are PM-only-shareable. The five investor templates — Investor One-Pager, Pitch Deck Outline, Market Opportunity, Traction Memo, Competitive Moat — are auto-marked at generation. Client-side UI hides the Share button and shows a Lock badge.',
        'Sharing never bypasses published_to_client. Documents must be PM-published before any share grant resolves. PM stays the gatekeeper of what enters client-side at all.',
        'Comments (document_comments) are threaded with one level of reply (Slack-style, not Reddit-nested). Section-scoped or doc-wide. Authorship tagged staff vs client, attribution always shows the named person. Resolve workflow: any commenter or PM can mark resolved. Soft-delete preserves thread context.',
        'Mentions parsed from comment body: format @[Display Name](staff:id) or @[Display Name](client:id). Mentioned users get a comment_mention notification. Replies notify the parent author. Client comments always notify the PM.',
        'Approvals (document_approvals) — PM requests approval per doc per version. State machine: in_review → approved | needs_changes | cancelled. PM picks a specific approver OR "any approver with grant" (notifies all approve-level grant holders). Client responds with optional message + optional link to a specific comment. Approval state syncs to project_documents.approval_state.',
        'Intake forms (client_intake_forms): PM-defined structured questionnaires. Six question types: short_text, long_text, single_choice, multi_choice, number, date. Each question can map to a Data Room category+field OR be free text. Visibility scoped by role. Status workflow: draft (hidden) → open (accepting) → closed (visible, no new responses).',
        'NEVER_OVERWRITE preserved on intake apply: when PM reviews a submission and applies it, mapped responses write to project_knowledge with source="client_intake" — BUT protected sources (manual, gsc_auto, ga4_auto, seed_migration) are never overwritten. PM sees skipped_protected in the review result with the existing source. Free-text answers become doc_type="intake_response" documents.',
        'Client uploads (bs_client_session_upload_file): routed through the existing bsIngestFile pipeline. Files tagged client_uploaded=true, published_to_client=false, approval_state=draft. They appear in PM\'s Ingest tab inbox for review + extraction before becoming visible to anyone else. 25MB cap with automatic image compression.',
        'In-app notifications (client_notifications): single table serves both staff and client recipients with discriminator. 11 kinds enumerated (comment_posted, comment_reply, comment_mention, approval_requested, approval_responded, share_granted, share_revoked, file_uploaded, intake_submitted, intake_applied, doc_published). Polled every 30s by the NotificationInbox bell.',
        'Audit log (bs_list_audit_log) — aggregates comments + approvals + share grants + intake submissions + uploads into a chronological merged stream. Every consequential client action is logged with named attribution + timestamp. Available to PM only.',
        'Document viewer (DocumentViewer): renders generated/ingested markdown content with proper formatting — headings, lists, tables, blockquotes, code, brand-color accents. Download as PDF (uses window.print with print stylesheet — A4, serif body, page-break-avoid on headings/tables). Download as .md. Copy to clipboard. Both PM Library and Client Workspace use it.',
        'Session-mode document fetch (bs_client_session_get_document): distinct from the legacy bare-token bs_client_get_document. Resolves via client_users.session_token, enforces share grants, returns full raw_content for reading. Without this fix, magic-link clients would fail to load doc details silently.',
        'PM-side disciplines visible in UI: hierarchy enforcement (Share dialog shows "You can only grant access at or below your own"), investor-grade lock badge, intake review showing NEVER_OVERWRITE protected fields with their existing source, notification bell badge counts on header.',
      ]},
      { title: 'Brand Studio — Stakeholders & Synthesis (H.5)', icon: '👥', items: [
        'Brand Studio → Stakeholders tab. Two stacked sections that turn the platform from a tool into a system of intelligence: Stakeholder Profiles + Persona Synthesis.',
        'Stakeholder Profiles capture named individuals per project — not just a role enum but actual people with attributes: communication preference (data-driven vs narrative), decision style (consensus-builder vs decisive), focus areas, what they care about most, language patterns, interaction history, watch-outs, preferred format.',
        'Best practice for stakeholder capture: 2-5 profiles per engagement. The decision-makers, key influencers, primary readers. Use real specifics from interactions — "Pushed back hard on X in March meeting" beats "values rigor".',
        'Persona Synthesis: pick 2-12 source documents (sales call notes, customer feedback, persona research, case studies). AI synthesizes a unified persona view across them — writes back to project_knowledge.audience.',
        'The synthesis discipline (most important feature): CONTRADICTIONS are surfaced EXPLICITLY rather than silently averaged. If Call 1 says budget is $50k and Call 3 says $200k, the synthesis lists both views with their source doc IDs and a recommendation like "verify with client" or "segment-specific finding". Never papers over conflicts.',
        'Each synthesized field carries: confidence (high/medium/low), evidence (array of source doc IDs that fed it), and reasoning (why the AI arrived at this value). All visible in the review UI.',
        'Protected values: synthesis NEVER overwrites manual / GSC / GA4 / seed_migration values. The review UI shows existing values for protected fields with a "protected" badge and the checkbox is disabled. The PM can still review the synthesis output for these fields, but must update manually in Data Room if they want to change them.',
        'Apply discipline: PM checks the boxes for fields to write, clicks Apply. Synthesis writes to project_knowledge with source="synthesis" (distinct badge in Data Room — would benefit from a future UI update to make this distinct from document_extracted).',
        'Versioning + diff view (H.5): every regeneration creates a v2 with parent_document_id linking to v1. Library shows a "Compare to v<n-1>" button on any doc with version > 1. Clicking opens a side-by-side modal showing every section as added / removed / modified / unchanged. Color-coded for quick scanning.',
        'Diff modal best practice: review the diff BEFORE clicking Publish. The Brand Studio rule is: never blind-publish a v2. Investors and clients notice when documents change without acknowledgment.',
        'Document re-extract (H.5): when the V2 field catalog evolves (we add new fields), existing ingested documents may carry undiscovered evidence. The re-extract action runs the ingest extraction again with the current schema. Available per-document.',
        'Dependency lookups (H.5): every generated document tracks what it subscribes to (auto-created from cited source IDs at generation time). A document\'s dependency view shows the specific Data Room fields, source documents, traction proof points, and market intelligence rows it depends on. Inverse lookup: pick a Data Room field, see all documents subscribing to it. Forensic and predictive both.',
        'Workflow integration: synthesis writes to audience fields → those fields can be cited by future Brand Statement / Audience Persona / Sales Battlecard generations → if synthesis is updated, those docs flag stale automatically via subscriptions → diff view shows what changed between regenerated versions. Full lifecycle closed.',
      ]},
      { title: 'Brand Studio — Market & Monitoring (H.4)', icon: '📡', items: [
        'Two new sub-tabs in Brand Studio: Market (configure URL monitors) and Triggers (review queue of meaningful changes + stale documents).',
        'Four monitor types: competitor_page (watch a competitor URL), industry_publication (watch a blog/news index for new posts), regulatory_source (watch a gov/regulator page), general_url (anything else).',
        'Every monitor requires a "Why" field — what specifically you\'re watching for and why a strategist would care. Forces intentional monitoring, prevents noise from random URL collection.',
        'Cron runs daily at 06:00 UTC. Each cron run picks up to 50 due monitors, fetches each, and runs AI change classification on any URL where the content hash differs from the previous check.',
        'The AI classification gate: every change is classified as no_change / cosmetic / meaningful / new_item / error. The default is COSMETIC (most edits — nav, footer, copy tweaks, A/B variations). Only MEANINGFUL and NEW_ITEM observations land in the open Triggers queue.',
        '"Would a senior brand strategist care?" — that is the explicit standard the AI uses. New products, pricing changes, leadership announcements, funding rounds, major positioning shifts, new customer logos, regulatory text updates → meaningful. Cookie banner tweaks → cosmetic. This noise filter is what makes monitoring usable.',
        'Suggested actions: meaningful observations include a specific suggested action and (when applicable) a specific template ID to regenerate. Click "Open <template>" to jump to the Generate tab with that template pre-selected.',
        'Suggestion not automation: the system never auto-regenerates anything. PM reviews each trigger and decides — Mark acted / Just reviewed / Dismiss.',
        'Hard cron cost caps: 50 fetches and 100 AI classifications per run. Predictable resource use. If queue depth exceeds caps, the cron logs monitors_remaining so you know your queue is bigger than the budget.',
        'Document subscriptions: every generated document automatically subscribes to its cited inputs (doc:* / traction:* / market_intel:* / dataroom:* IDs). When a subscribed input meaningfully changes, the document is flagged stale.',
        'Stale documents appear in two places: (a) Library shows an orange "Inputs changed" badge on each affected doc with a tooltip listing why; (b) Triggers tab has a stale-docs queue with the specific input changes and a Regenerate button per doc.',
        'Check Now button: manually trigger a check on any monitor without waiting for cron. Useful for high-priority monitors after you\'ve just added them, or when you suspect a competitor is making a move.',
        'Error handling: monitors with consecutive_errors > 0 show a red badge. Common causes: URL returns 403 (bot detection), URL no longer exists, timeout on slow pages. Fix the URL or disable the monitor.',
        'Monitor frequency presets: 6h / 12h / 1d / 3d / 1w / 30d. Default 24h is fine for most. Use higher frequency only where same-day awareness matters (e.g. competitor pricing pages during their announced launch week).',
        'Best practice cadence: review the Triggers queue weekly (e.g. Monday morning). Configure 5-15 high-quality monitors per project — competitor pricing/positioning pages, top industry publications, key regulatory sources. Resist the urge to monitor 50+ URLs; quantity destroys signal.',
      ]},
      { title: 'Brand Studio — Investor View (H.3)', icon: '💼', items: [
        'Brand Studio → Investor tab. The single biggest credibility layer in the platform: capture investor-grade evidence once, then have every investor template generate from it with full citations.',
        'Three sub-tabs: Traction Proof Points (dated, sourced performance claims), Market Intelligence (TAM/SAM/SOM/competitor data with URL citations), Research (server-side URL fetch + citation extraction from trusted sources only).',
        'Five investor-grade templates plug into this data: Investor One-Pager (90-second first-look), Pitch Deck Outline (slide-by-slide with speaker notes), Market Opportunity Memo (Series-B-grade market sizing), Traction Memo (quarterly investor update), Competitive Moat Memo (diligence-stage defensibility argument).',
        'Traction Proof Points: every entry has category, claim, metric_value, evidence_date, evidence_type (verified_third_party / verified_internal / self_reported / estimate), confidence, status (draft/verified/archived). Investor templates source from these via citation IDs like "traction:abc123".',
        'Best practice for traction: only mark "verified" when you would be comfortable defending the data point in a diligence call. Verified rows are what clients see in their workspace; drafts stay PM-side. Cap the number of verified proof points to 10-15 of the highest-quality — fewer high-quality is more credible than many medium-quality.',
        'Market Intelligence: every entry has category (tam/sam/som/growth_rate/market_share/competitor_funding/competitor_metric/industry_trend/regulatory/other), claim, metric_value, source_url, source_name, source_date, methodology, assumptions, confidence. URL is REQUIRED for high confidence — without one, confidence is capped at medium when saved.',
        'Source quality discipline: investor docs cite three approved source types — government statistics (.gov, OECD, World Bank, BLS, Eurostat), reputable industry research firms (Gartner, Forrester, IDC, McKinsey, BCG, Statista, etc.), and primary company filings (SEC, Companies House, Crunchbase for funding). Default research allowlist enforces this.',
        'Research tab: paste a URL + your query (what you\'re looking for on that page), system fetches server-side, cleans HTML, extracts excerpts that match query terms. Copy excerpts directly into Market Intelligence entries\' source_excerpt field. Default: trusted-domain allowlist. Override available with explicit "untrusted reason" for legitimate non-allowlist sources.',
        'Generation engine integration: when generating ANY investor template, the engine automatically pulls all non-archived traction_proof_points + market_intelligence into the prompt context, alongside Data Room + ingested docs + brand assets. The AI cites them with strict IDs (traction:* and market_intel:*). NEVER fabricates market data.',
        'Investor-grade strictness amplified: investor templates already had the "sections without sources get downgraded to low confidence" rule. With traction/market intel as cited inputs, sections that USE these sources get genuinely high confidence — making the difference visible.',
        'Client portal investor view: when investor entitlement is enabled for a project, the /c/<token> client workspace shows a populated Investor tab with all verified traction + market intelligence rows. Read-only, beautifully laid out, branded with the client\'s own colors. This is what founders share with their investors directly.',
        'Workflow for a Series-A founder client: PM captures 10-15 verified traction proof points + 15-20 market intelligence rows with URLs + ingests their pitch deck → generates Investor One-Pager, Traction Memo, Pitch Deck Outline → publishes to client workspace → founder shares /c/<token> link with their existing investors for quarterly updates and with new investors for first-look outreach. The entire IR motion lives in one place with one citation discipline.',
        'PM hygiene: review the Investor tab quarterly. Archive stale market intel where source date is over 18 months old. Add new traction proof points after every quarter. Keep verified count tight — quality dominates quantity for investor scrutiny.',
      ]},
